...
// La couche données bridge ses `_sb*` sur window (effet de bord de l'import) ; on importe
// nommément les loaders déclenchés au login + save/delete module (appelés par app.js).
import { sb, SUPABASE_CONFIGURED, DEV_SKIP_AUTH } from './lib/supabase-client.js';
import {
...
// Les `_sb*` de DONNÉES sont désormais bridgés par lib/supabase-data.js (leur nouveau
// module). Ici : helpers app-level, dont _cache/updateStudentBar que la couche données
// appelle via window (elle vit dans un autre module maintenant).
Object.assign(window, {
  _buildDrillTree, _commentDelay, _commentWithShapes, _drillSessions, _findNodeByFen,
  _materialHint, _nagGroup, _parseShapes, _shapesToPGN, _treePlayerPositions,
  _cache, updateStudentBar,
