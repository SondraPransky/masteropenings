...
function renderHeatmap() {
  ...
  const sections = [sel].map(m => {
    ...
    const rowsHtml = shown.map(p => {
      ...
      // ⚠ 8 pastilles par rangee = 45 noms a l'ecran sur 5 lignes, sur DEUX lignes
      // chacune : la colonne noyait la table. Le coach a besoin de savoir COMBIEN
      // et QUI en tete — la liste complete vit dans la modale « Voir la position ».
      const chips = fails.slice(0,3).map(s=>`<span class="wsx-chip">${escapeHtml(s)}</span>`).join('')
        + (fails.length>3 ? `<span class="wsx-chip wsx-chip-more" title="${escapeHtml(fails.slice(3).join(', '))}">+${fails.length-3}</span>` : '');
      return `<tr class="wsx-tr${p.rate<50?' hot':''}" tabindex="0" data-did="${escapeHtml(String(p.drillId))}" data-san="${escapeHtml(p.san)}"
        ...
        <td><span class="mono-move">${fig(p.san)}</span>
          ${window.oaaLichessNote?.(p.drillId, p.san) || ''}</td>
  ...
  el.innerHTML = `
    ...
    <!-- « Survolez » excluait le clavier : chaque rangée est tabbable et pose
         l'aperçu au focus (onfocus/onblur). Même correction qu'en analyse d'ouvertures. -->
    <div class="wsx-hint"><i class="ti ti-pointer" aria-hidden="true"></i> Choisissez une ligne pour voir la position sur l'échiquier.</div>
    ${sections}`;
}
...
function wsTip(event, drillId, san) {
  ...
  // ⚠ La rangée occupe TOUTE la largeur de la table : l'ancrer sur son bord droit
  // envoyait systématiquement le tooltip à gauche de l'écran, par-dessus la nav
  // coach. On l'ancre au POINTEUR (mouseenter ne tire qu'une fois : la position
  // ne suit pas la souris ensuite) et on retombe sur le début de la rangée au
  // focus clavier. Borne gauche = le bord de la rangée, jamais la nav.
  const ax = event.clientX > 0 ? event.clientX : r.left + 40;
  const hi = Math.max(8, window.innerWidth - tw - 8);
  // Sur une rangée LARGE (page profil : coup + méta à gauche, vide au milieu), le
  // plancher gauche saute la zone de lecture — survoler le coup posait l'aperçu
  // PAR-DESSUS la colonne des coups des rangées voisines (retour 03/08).
  const lo = Math.min(Math.max(8, r.left + (r.width > 640 ? 320 : 0)), hi);
  let x = ax + 24;
  if (x > hi) x = ax - tw - 24;
  x = Math.min(Math.max(x, lo), hi);
  let y = Math.round(r.top + r.height / 2 - th / 2);
  y = Math.min(Math.max(8, y), Math.max(8, window.innerHeight - th - 8));
  tip.style.left = x + 'px';
  tip.style.top  = y + 'px';
}
function wsTipHide() {
