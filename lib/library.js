...
import { fig, moveLabel, pgnMainlineSans, pgnStartFen } from './core.js';
...
function deleteBase(id) {
  ...
  // ⚠ Supprimer aussi CHAQUE partie côté Supabase (audit 03/08) : sans ça, le retrait
  // n'était que local — au prochain login les parties revenaient du cloud, orphelines
  // (invisibles dans la bibliothèque mais toujours partagées au coach).
  _baseGames(id).forEach(g => window._sbDeleteGame?.(g.id));
...
// ── Comparer au répertoire (arbitrage 28/07 : l'élève VOIT sa sortie) ───────
// Chaque partie est confrontée aux arbres des modules de l'élève (assignés +
// perso). Mémoïsé par partie ET par état des modules (une édition de module
// invalide la comparaison). Le rendu : la sortie en ambre, le suivi en vert —
...
function _repInfo(g) {
  ...
  if (_repCache.has(key)) return _repCache.get(key);
  if (_repCache.size > 300) _repCache.clear();
  let r = null;
...
function _repLineHTML(g) {
  const r = _repInfo(g);
  if (!r) return '';
  if (r.exit) {
    const num = moveLabel(r.exit.moveNo, r.exit.color);
    const exp = r.exit.expected.map(s => fig(escapeHtml(s))).join(' ou ');
    return `<div class="lib-entry-rep warn"><i class="ti ti-route-off" aria-hidden="true"></i>
      Sortie du répertoire au coup ${num} : ${fig(escapeHtml(r.exit.played))} — le répertoire joue ${exp}
      <span class="lib-entry-rep-mod">(${escapeHtml(r.d.name)})</span></div>`;
  }
  const nCoups = Math.ceil(r.depth / 2);
  return `<div class="lib-entry-rep ok"><i class="ti ti-route" aria-hidden="true"></i>
    Suit « ${escapeHtml(r.d.name)} » sur ${nCoups} coups</div>`;
}
...
function _baseDetailHTML(b) {
  ...
  const etabli = `<div class="lib-etabli">
    ...
    <div class="lib-qe-right">
      <div class="lib-qe-hint" id="lib-qe-hint">Joue des coups pour saisir une partie — ou pour chercher : la liste se limite aux parties qui passent par la position. Tu recopies une feuille ? Tape les coups en notation française (Cf3, Fxc4…).</div>
      <form class="lib-qe-type" onsubmit="return qeTypeSubmit(event)">
        <span class="lib-qe-turn" id="lib-qe-turn">Coup 1 — Blancs</span>
        <input id="lib-qe-san" type="text" autocomplete="off" autocapitalize="off" spellcheck="false"
               placeholder="Cf3, e4, O-O…" aria-label="Taper un coup en notation (française ou anglaise)">
        <button type="submit" class="btn btn-ghost btn-sm" aria-label="Jouer le coup tapé"><i class="ti ti-corner-down-left" aria-hidden="true"></i></button>
      </form>
...
function saveGameEntry() {
  ...
  // Feedback immédiat à la saisie (arbitrage 28/07) : la sortie de répertoire
  // se dit tout de suite — c'est le moment où l'élève se souvient de la partie.
  const cmp = _repInfo(rec);
  if (cmp?.exit) {
    const num = moveLabel(cmp.exit.moveNo, cmp.exit.color);
    toast(`Tu as quitté le répertoire au coup ${num} (${cmp.exit.played} au lieu de ${cmp.exit.expected.join(' / ')}) — regarde « ${cmp.d.name} »`, 'ok');
  } else if (cmp && cmp.depth >= 8) {
    toast(`Bien joué : la partie suit « ${cmp.d.name} » sur ${Math.ceil(cmp.depth / 2)} coups`, 'ok');
  }
}
