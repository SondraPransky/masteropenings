  ...
  const cards = bases.map(b => {
    ...
    return `<div class="lib-base" onclick="openBase('${b.id}')">
      ...
      <div class="lib-base-actions">
        ...
        <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();renameBase('${b.id}')" title="Renommer" aria-label="Renommer la base">✏️</button>
        <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();deleteBase('${b.id}')" title="Supprimer" aria-label="Supprimer la base">🗑</button>
      </div>
    </div>`;
  }).join('');
  const empty = `<div class="lib-empty">
    <div class="lib-empty-ico">🗄️</div>
...
function _baseDetailHTML(b) {
  ...
  const list = games.length
    ? `<div class="lib-list">${games.map(g => {
        ...
        const reviewed = g.reviewedAt ? `<span class="lib-chip-coach" title="Annotée par le coach">✨ Annotée</span>` : '';
        const shareBtn = g.shared
          ...
          : `<button class="btn btn-ghost btn-sm" onclick="toggleShareGame('${g.id}')" title="Rendre visible par le coach">📤 Partager</button>`;
        return `<div class="lib-entry">
          <div class="lib-entry-result ${resClass(g.result)}">${escapeHtml(g.result || '*')}</div>
          <div class="lib-entry-main">
            <div class="lib-entry-title">${escapeHtml(title)} <span class="nat">${natLabel[g.nature] || '♟️'} ${natTxt}</span></div>
            <div class="lib-entry-meta">${bits}${bits && reviewed ? ' ' : ''}${reviewed}</div>
          </div>
          <div class="lib-entry-actions">
            <button class="btn btn-primary btn-sm" onclick="openGameReview('${g.id}')">${g.reviewedAt ? '📖 Voir la revue' : '🔎 Revoir'}</button>
            ${shareBtn}
            <button class="btn btn-ghost btn-sm" onclick="deleteGame('${g.id}')" title="Supprimer" aria-label="Supprimer">🗑</button>
          </div>
        </div>`;
      }).join('')}</div>`
    : `<div class="lib-empty">
  ...
  return `
    ...
    <div class="lib-detail-sub">${games.length} entrée${games.length > 1 ? 's' : ''}</div>
    ${list}`;
}
