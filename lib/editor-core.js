...
export function _parseShapes(raw) {
  ...
  return { text: s.replace(/\s+/g,' ').trim(), shapes, author };
}
...
export function _commentWithShapes(node) {
  const au = node.author === 'coach' ? '[%author coach]' : '';   // P1.4 : marque additive du coach
  const sh = _shapesToPGN(node), cm = node.comment || '';
  return [au, sh, cm].filter(Boolean).join(' ');
}
...
export function pgnToEditorTree(pgn, startFen) {
  ...
  while ((m = re.exec(text)) !== null) {
    if (m[1] !== undefined) {
      ...
      if (ps.text || ps.shapes.length || ps.author) tokens.push({type:'comment', text:ps.text, shapes:ps.shapes, author:ps.author});
