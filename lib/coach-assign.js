...
function assignReviewForStudent(btn) {
  ...
  const res = _assignReviewCore(
    // La rangée peut porter la position EXACTE (data-fen, cf. sorties de répertoire).
    // Sinon seulement, on retombe sur _drillFenMap — qui collapse san → fen sur
    // tout l'arbre du module et rendrait la première position portant ce SAN.
    { drillId, drillName: drill ? drill.name : '', san,
      fen: it.dataset.fen || _drillFenMap(drillId)[san] || null, comment: hit ? hit.comment : '' },
    (cls, roster) => {
