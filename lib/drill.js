...
function showEndModal(pct) {
  ...
  if (errBtn) {
    ...
    if (hasTreeErrors) {
      ...
      errBtn.innerHTML = `<i class="ti ti-arrow-back-up" aria-hidden="true"></i> Réviser les erreurs (${S.ko})`;
    }
  }
  ...
  // Hiérarchie : s'il y a des erreurs, l'action primaire (seul plein indigo) est de
  // LES RÉVISER — pas « Continuer la partie » (tonal) ni « Module suivant » (démoté).
  if ((hasLineErrors || hasTreeErrors) && errBtn) {
    errBtn.className = 'btn btn-primary';
    errBtn.style.color = '';
    if (nextBtn) { nextBtn.className = 'btn btn-ghost'; nextBtn.style.color = 'var(--dim)'; }
  }
