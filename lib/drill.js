...
import { isPlayerMove, _materialHint, _buildDrillTree, chapterCount, chapterPgn } from './tree.js';
import { _normFen, extractAllLines } from './core.js';
...
function loadPosition(posIdx) {
  ...
  // (carte recto/verso), PUIS test — façon Chessable. Marquée _taught pour ne pas
  // re-flasher aux occurrences suivantes. `_reteach` (erreur requeue / rejeu des
  // erreurs) repasse aussi par la carte — ré-enseigner avant de re-tester.
  const kp = S.kps[posIdx];
  if ((S.sr && S.sr.active && kp._srNew && !kp._taught) || (kp._reteach && !kp._taught))
    _srShowFlash(kp, kp._reteach ? 'reteach' : 'new');
}

// ── CARTE FLASH recto/verso — LE format du savoir en mode positions ──────────
// Recto : « Nouvelle position · [camp] joue » (la position reste visible, rien
// n'est spoilé). Verso : le coup en grand + la flèche sur l'échiquier + le
// commentaire du coach. Le plateau reste VERROUILLÉ pendant le flash (S.srFlash,
// board.js) ; la flèche est dessinée par drawBoard via S.flashArrow (autoShapes).
// La même carte (teinte erreur, sans flip) sert de correction — voir _flashCorrection.

// from/to du coup attendu AU PLY COURANT (multi-coups : la solution repart de la
// position d'échec, pas du FEN de départ — sinon la flèche serait fausse dès ply 2).
function _kpExpectedMove(kp) {
  try {
    const multi = kp.line && kp.line.length > 1;
    const fen = (multi && S.exPly && S.game) ? S.game.fen() : kp.fen;
    const san = multi ? kp.line[S.exPly || 0] : kp.san;
    const tmp = new Chess(fen);
    const mv = tmp.move(san, { sloppy: true });
    return mv ? { from: mv.from, to: mv.to } : null;
  } catch (e) { return null; }
}
// Solution côté élève, en figurines (multi-coups : ses coups à lui, séparés par …).
function _kpSolutionHTML(kp) {
  return (kp.line && kp.line.length > 1)
    ? kp.line.filter((_, i) => i % 2 === 0).map(fig).join(' <span class="fcard-ellip">…</span> ')
    : fig(kp.san);
}
function _kpSideLabel(kp) {
  try { return new Chess(kp.fen).turn() === 'w' ? 'Les Blancs jouent' : 'Les Noirs jouent'; }
  catch (e) { return ''; }
}
function _fcardComment(kp) {
  return (kp.comment && !S.drill?.hideComments)
    ? `<div class="fcard-comment">${figText(escapeHtml(kp.comment))}</div>` : '';
}

// Affiche la carte d'apprentissage (mode 'new' = ✨ nouvelle · 'reteach' = ↻ revue).
function _srShowFlash(kp, mode) {
  S.srFlash = true;
  S.flashArrow = null;
  S.hintSquare = null;
  drawBoard();
  const el = document.getElementById('feedback');
  if (!el) return;
  const eyebrow = mode === 'reteach'
    ? '<i class="ti ti-rotate" aria-hidden="true"></i> On la revoit'
    : '✨ Nouvelle position';
  el.className = 'feedback fcard-host';
  el.innerHTML = `<div class="fcard" id="flash-card" data-face="recto"><div class="fcard-inner">
    <div class="fcard-face fcard-front">
      <div class="fcard-eyebrow">${eyebrow}</div>
      <div class="fcard-side">${_kpSideLabel(kp)}</div>
      <button class="btn btn-primary fcard-btn" onclick="srFlashFlip()">
        Retourner la carte <i class="ti ti-rotate-360" aria-hidden="true"></i></button>
    </div>
    <div class="fcard-face fcard-back">
      <div class="fcard-eyebrow">${eyebrow}</div>
      <div class="fcard-move">${_kpSolutionHTML(kp)}</div>
      ${_fcardComment(kp)}
      <button class="btn btn-primary fcard-btn" onclick="srFlashDone()">À toi de le jouer →</button>
    </div>
  </div></div>`;
}
// Retourne la carte : le verso apparaît ET la flèche du coup se dessine sur l'échiquier.
function srFlashFlip() {
  const kp = S.kps[S.posIdx]; if (!kp) return;
  S.flashArrow = _kpExpectedMove(kp);
  drawBoard();
  const card = document.getElementById('flash-card');
  if (card) card.dataset.face = 'verso';
}
...
// Correction d'une erreur au format carte (verso direct, teinte erreur) — B3.
// opts.hint = indice matériel · opts.next = bouton « Continuer » (chemins qui
// avançaient en minuterie : SR, multi-coups) · sans next = l'élève rejoue le bon
// coup pour avancer (mode positions simple, comportement inchangé).
function _flashCorrection(kp, opts = {}) {
  S.flashArrow = _kpExpectedMove(kp);
  drawBoard();
  const el = document.getElementById('feedback');
  if (!el) return;
  el.className = 'feedback fcard-host';
  el.innerHTML = `<div class="fcard ko"><div class="fcard-inner">
    <div class="fcard-face">
      <div class="fcard-eyebrow">✗ Pas tout à fait${opts.requeued ? ' · elle reviendra dans un instant' : ''}</div>
      <div class="fcard-move">${_kpSolutionHTML(kp)}</div>
      ${opts.hint ? `<div class="fcard-hint">${escapeHtml(opts.hint)}</div>` : ''}
      ${_fcardComment(kp)}
      ${opts.next
        ? `<button class="btn btn-primary fcard-btn" onclick="_flashCorrectionNext()">Continuer →</button>`
        : `<div class="fcard-side">À toi de le jouer</div>`}
    </div>
  </div></div>`;
  // Reflet sous l'échiquier (le feedback vit dans la sidebar → sous le plateau au téléphone).
  window.setBoardComment?.(S.drill?.hideComments ? '' : (kp.comment || ''));
  window.setBoardPrompt?.('ko', '✗ Le bon coup : ' + fig(kp.san));
}
function _flashCorrectionNext() {
  S.flashArrow = null;
...
function tryMoveInPositions(from, to) {
  ...
  if (isCorrect) {
    ...
    S.ok++;
    // `kp.isCheck` fusionne echec ET mat (`/\+|#/` dans exercises.js) : tout mat
    // resolu s'annoncait « — echec ! », y compris sur un paquet de tableaux de mat.
    // Le mat se lit dans le SAN lui-meme (`#`), donc pas de drapeau a propager ni
    // de paquet deja enregistre a migrer. Il PRIME la capture : sur `Bxe7#` la
    // bonne nouvelle est le mat, pas la piece prise.
    const _mate = /#\s*$/.test(played.san);
    S.flashArrow = null;   // une correction précédente a pu poser la flèche
    setFeedback('ok',
      '✓ Correct ! '+fig(played.san)+(_mate?' — mat !':kp.isCapture?' — bonne capture !':kp.isCastle?' — roque !':kp.isCheck?' — échec !':''),
      S.drill.hideComments ? '' : kp.comment);
    addLog(played.san, true, S.posIdx+1);
  } else {
    S.ko++;
    const matHint = _materialHint(S.game.fen(), played.san);
    // Correction au format carte (B3) : bon coup + flèche + commentaire. Pas de
    // bouton : comme avant, l'élève rejoue le bon coup pour avancer.
    _flashCorrection(kp, { hint: matHint });
...
function _exSeqMove(played, kp) {
  ...
  setTimeout(() => {
    ...
    // Garde (audit 03/08) : une réplique scriptée ILLÉGALE (ligne mal saisie par le
    // coach) coinçait le drill — exPly avançait quand même, « À toi de jouer » avec le
    // trait à l'adversaire, plateau mort. On clôture proprement : l'élève a joué juste
    // jusqu'ici, la séquence compte résolue.
    if (!m) { S.exWaiting = false; _exSeqFinalize(kp, true, null); return; }
...
function _exSeqFinalize(kp, isCorrect, played) {
  ...
  if (isCorrect) {
    ...
    S.ko++;
    // Correction au format carte (B3). Le plateau a bougé → pas de re-essai sur
    // place : bouton « Continuer » au lieu de la minuterie (l'élève lit à son rythme).
    _flashCorrection(kp, { next: true });
    addLog((played ? played.san : '?') + ' ✗', false, S.posIdx + 1);
  }
  updateScores(); renderPosStrip(); drawBoard();
  recordResult(isCorrect, { san: kp.san, comment: kp.comment, posIdx: S.posIdx });
  if (isCorrect) setTimeout(() => loadPosition(S.posIdx + 1), 1200);
}
...
function startTreeDrill() {
  ...
  S.lineGame = new Chess(sess?.startFen || new Chess().fen());
  S.ok = 0; S.ko = 0; S.sel = null;
  S.waitingForPlayer = false;
  S.phase = 'tree';
  // Module a CHAPITRES : le drill travaille sur l'arbre du SEUL chapitre courant,
  // reconstruit a la volee depuis sa partie du PGN. Les chapitres se chevauchent
  // (le ch.2 demarre sur la ligne du ch.1) : sur l'arbre FUSIONNE, le chapitre 1
  // avalerait tout le fichier et « Chapitre 1/5 terminé » serait un mensonge.
  S.chapterTree = null;
  if (chapterCount(d) > 1) {
    try { S.chapterTree = _buildDrillTree(extractAllLines(chapterPgn(d, S.sessionIdx)), d.side); }
    catch (e) { S.chapterTree = null; }   // repli : arbre fusionné (drill jouable, bornage perdu)
  }
  S._forcedPath = _computeForcedPath(S.student || '', String(d.id ?? ''), _drillTree(), d.side);
  S._treeErrors = [];
...
// Arbre sur lequel le drill travaille : celui du chapitre courant si le module
// en a (S.chapterTree, posé par startTreeDrill), sinon l'arbre fusionné du module.
function _drillTree() {
  return S.chapterTree || S.drill?.tree || {};
}
...
function _treeUnseenCount() {
  ...
  return treeUnseenCount(_drillTree(), S.student || '', String(S.drill?.id ?? ''), G.oppSeen);
}
...
function tryMoveInTree(from, to) {
  ...
  if (isValid) {
    ...
    updateScores(); setFeedback('ok', `✓ ${fig(legal.san)}`, S.drill.hideComments ? '' : (isValid.comment||'')); drawBoard();
    ...
    // Figurines : 10 des 12 sites de feedback passent par fig(), les deux du mode
    // ARBRE ne le faisaient pas — or c'est le mode de tout le repertoire d'ouverture,
    // donc la chaine la plus lue de l'app sortait en SAN brut (« Nf3 » pour « ♘f3 »).
    const expected = node.player.map(m => fig(m.san)).join(' / ');
    const corrComment = node.player[0].comment||'';
    const matHint = _materialHint(g.fen(), legal.san);
    setFeedback('ko', `✗ ${fig(legal.san)} — attendu : ${expected}${matHint ? ' · ' + matHint : ''}`, S.drill.hideComments ? '' : corrComment); drawBoard();
...
function showEndModal(pct) {
  ...
  // Module a CHAPITRES : la fin d'une traversee est la fin d'un CHAPITRE, pas du
  // module — le titre et le bouton « suivant » suivent la progression sequentielle.
  const _nChap    = chapterCount(S.drill);
  const _isChap   = S.drill?.varmode === 'tree' && _nChap > 1;
  const _chapDone = _isChap && _treeUnseenCount() === 0;
  const _hasNext  = _isChap && S.sessionIdx < (S.drill.sessions?.length || 1) - 1;
  const _et = document.getElementById('end-title');
  if (_et) _et.textContent = flawlessEx ? 'Sans faute !'
    : _isChap ? `Chapitre ${S.sessionIdx + 1}/${S.drill.sessions.length}${_chapDone ? ' terminé !' : ''}`
    : '🏁 Module terminé !';
  const msg = flawlessEx ? _EX_DONE_LINES[Math.floor(Math.random() * _EX_DONE_LINES.length)]
            // « Maitrise parfaite » etait servi des 85 %, donc juste a cote d'un
            // « 1 Erreur » affiche dans la meme rangee de scores. On la reserve au
            // sans-faute reel ; 85-99 % reste excellent, sans se contredire.
            : S.ko===0&&S.ok>0?'🌟 Excellent ! Maîtrise parfaite.':pct>=85?'🌟 Excellent !':pct>=60?'👍 Bon travail, continuez !':'💪 Persistez, vous progressez !';

  let errRecap = '';
  if (isLineMode() && S.ko > 0 && S.lineAllMoves?.length) {
    const failed = S.lineAllMoves.filter(m => m.isPlayer && m.result === 'ko');
    if (failed.length) {
      errRecap = `<div style="margin:12px 0 0;padding:10px 12px;background:var(--red-dim);border:1px solid var(--red-glow);border-radius:var(--rs);text-align:left">
  ...
  document.getElementById('end-body').innerHTML=`
    ${flawlessEx ? `<div style="display:flex;align-items:center;justify-content:center;gap:14px;margin:-4px 0 10px">${mascotSvg(56, 'cheer')}${_EX_DONE_CHECK}</div>` : ''}
    <div class="scores-row" style="margin-bottom:14px">
      <div class="score-box"><div class="score-val" style="color:var(--green)">${S.ok}</div><div class="score-lbl">Correct${S.ok > 1 ? 's' : ''}</div></div>
      <div class="score-box"><div class="score-val" style="color:var(--red)">${S.ko}</div><div class="score-lbl">Erreur${S.ko > 1 ? 's' : ''}</div></div>
  ...
  // RÉVISER — « Module suivant » est alors démoté.
  ...
  if (nextBtn) {
    ...
    if (_chapDone && _hasNext) {
      // Chapitre epuise, il en reste : la suite naturelle est le chapitre suivant
      // (parcours sequentiel — nextSession relance l'apprentissage du nouveau chapitre).
      label = 'Chapitre suivant →'; primary = !hasErrors;
      onclick = () => { closeModal('modal-end'); window.nextSession?.(); };
    }
    else if (unseen > 0) { label = `${unseen} variante${unseen > 1 ? 's' : ''} restante${unseen > 1 ? 's' : ''}`; primary = false; onclick = restart; }
...
function replayErrors() {
  if (S.drill?.varmode === 'tree') {
    ...
    // `_reteach` : chaque erreur repasse d'abord par la carte flash (« On la
    // revoit ») avant le re-test — B1, même dehors d'une session SR.
    S.kps = errors.map(e => ({ fen: e.fen, san: e.san, comment: e.comment, attempted: false, correct: false, _reteach: true }));
    ...
    renderPosStrip();
    // ⚠️ Pas de bandeau « N erreurs à corriger » après loadPosition : il écraserait
    // la carte flash « On la revoit » que la 1re position affiche désormais (B1).
    loadPosition(0);
    return;
  }
  const failed = (S.lineAllMoves||[]).filter(m => m.isPlayer && m.result === 'ko');
...
Object.assign(window, {
  ...
  srFlashFlip, _flashCorrection, _flashCorrectionNext,
