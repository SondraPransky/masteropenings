...
function renderOverview() {
  ...
  const sharedN    = G.savedGames.filter(g => g.baseId && g.shared).length;
  kpiEl.innerHTML = `
    ...
    <div class="cs-kpi-note">${students.length?`<i class="ti ti-refresh" aria-hidden="true"></i> ${activeWk} actif${activeWk>1?'s':''} cette semaine · ${inactive} inactif${inactive>1?'s':''} (&gt;7j)`:''}<span class="cs-kpi-note-sep">${G.practiceLog.length} session${G.practiceLog.length>1?'s':''} · ${sharedN} partie${sharedN>1?'s':''} reçue${sharedN>1?'s':''}</span></div>`;
