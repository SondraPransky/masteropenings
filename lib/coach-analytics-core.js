...
export const OA = {
  ...
  // Panneau de paramètres (rails, patron demandé par l'utilisatrice le 22/07) —
  // persistés ensemble dans mc_oaa_params.
  side: 'all',        // camp fautif : 'all' | 'w' | 'b' (stm de l'erreur)
  minLoss: 0,         // coût minimal en centipions (0 = tous) : 0|25|50|100|200
  topCrit: 0,         // ne garder que les N plus critiques (0 = toutes) : 0|50|20|10
};
...
// ── Filtre des erreurs (le doc est déjà trié par criticality). ──
...
// `opts` (panneau de paramètres) : side = camp fautif (stm de l'erreur),
// minLossCp = coût plancher en centipions. Chaque axe absent = neutre.
export function filterErrorsIndexed(errors, bucket, opts = {}) {
  const { side = 'all', minLossCp = 0 } = opts;
  return (errors || []).map((e, i) => ({ e, i }))
    .filter(({ e }) => (bucket === 'all' || e.bucket === bucket)
      && (side === 'all' || e.stm === side)
      && (!minLossCp || (e.lossCp ?? 0) >= minLossCp));
}

export function filterErrors(errors, bucket, opts) {
  return filterErrorsIndexed(errors, bucket, opts).map(x => x.e);
}

// ── Plafond « les N plus critiques » (pur, sur les paires {e,i}). ──
// Sélectionne l'ENSEMBLE par rang de criticité, mais restitue les survivants
// dans l'ordre reçu : « top 10 » compose avec n'importe quel tri d'affichage
// (trié par coût, on voit les 10 plus critiques, ordonnées par coût).
export function capTopCrit(indexed, n) {
  if (!n || indexed.length <= n) return indexed;
  const keep = new Set([...indexed]
    .sort((a, b) => (b.e.crit ?? 0) - (a.e.crit ?? 0))
    .slice(0, n).map(x => x.i));
  return indexed.filter(x => keep.has(x.i));
}
...
export function sortErrors(indexed, key, dir) {
  const val = e => key === 'freq' ? (e.freq ?? 0)
    : key === 'lossCp' ? (e.lossCp ?? 0)
    // Trier PAR TRANCHE regroupe les erreurs d'un meme niveau : c'est la vue
    // « qu'est-ce que je corrige chez mes 1600 ? ». Le tri stable conserve, a
    // l'interieur d'une tranche, l'ordre du doc (criticite decroissante).
    : key === 'bucket' ? (e.bucket ?? 0)
    : (e.crit ?? 0);
