...
function exportPGN(idx) {
  ...
  // G.savedGames melange DEUX natures : les parties vs Maia (drillName/side/student)
  // et les entrees de bibliotheque partagees par les eleves (white/black/event).
  // L'en-tete etait reconstruit pour la seule forme Maia -> une partie de tournoi
  // d'eleve s'exportait en `[Event "undefined"] [White "Maia"] [Black "Maia"]`.
  // On prefere donc les vrais noms quand la fiche les porte.
  const esc = v => String(v ?? '').replace(/"/g, "'");
  const out = games.map(g => {
    const isMaia = !g.baseId;
    const ev    = g.event  || (isMaia ? g.drillName : null) || 'Partie';
    const white = g.white  || (g.side === 'w' ? g.student : 'Maia');
    const black = g.black  || (g.side === 'b' ? g.student : 'Maia');
