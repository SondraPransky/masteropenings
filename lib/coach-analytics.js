...
import {
  ...
  filterErrorsIndexed, capTopCrit, sortErrors, oaaFenIndex,
...
function renderOaAnalytics() {
  ...
  if (!body) return;
  // Paramètres mémorisés d'une session à l'autre (posés une seule fois).
  if (!OA._bucketRead) {
    OA._bucketRead = true;
    try { const b = localStorage.getItem('mc_oaa_bucket'); if (b) OA.bucket = b; } catch (e) {}
    try {
      const p = JSON.parse(localStorage.getItem('mc_oaa_params') || '{}');
      if (p.side === 'w' || p.side === 'b') OA.side = p.side;
      if (Number.isFinite(p.minLoss)) OA.minLoss = p.minLoss;
      if (Number.isFinite(p.topCrit)) OA.topCrit = p.topCrit;
    } catch (e) {}
  }
  ...
  // Les chips de tranche ont quitté la barre : la tranche vit désormais dans le
  // panneau de paramètres de l'onglet Erreurs (rail « Tranche »), et Trous /
  // Diagnostics portent leurs propres chips — chaque réglage habite là où il agit.
  ...
  body.innerHTML = `
    <div class="oaa-bar">
      ...
      <span class="oaa-fresh">analysé le ${_oaaDate(cur.updatedAt)} · ${_oaaTotalsLabel(doc)}</span>
...
// Le doc porte totals = { errors: toutes les erreurs trouvées, kept: celles
// retenues après plafonds } — la table affiche les retenues, le bandeau doit
// dire le même chiffre (« 390 détectées » face à « /137 » était un mensonge).
function _oaaTotalsLabel(doc) {
  const kept = doc.totals?.kept ?? doc.totals?.errors ?? 0;
  const all = doc.totals?.errors ?? kept;
  return all > kept ? `${kept} erreurs retenues (sur ${all} détectées)` : `${kept} erreurs détectées`;
}
...
function _oaaRerenderContent() {
  ...
  // (Les chips de tranche vivent désormais DANS #oaa-content → reconstruites avec
  // lui ; seuls les onglets, hors contenu, se synchronisent en place.)
...
// ── Panneau de paramètres : un RAIL par axe (patron demandé par l'utilisatrice,
// réf. vidéo du 22/07 — rangées de crans, pilule qui se remplit jusqu'au cran).
// Chaque rangée est un radiogroup ; la pilule est un pur indicateur de position
// (comme un curseur à crans), pas une quantité. Tout re-rend #oaa-content : les
// crans actifs n'ont pas besoin de synchro en place.
function _oaaParamsHTML(doc, shown, total) {
  const rail = (label, name, stops, cur) => {
    const k = Math.max(0, stops.findIndex(s => String(s.v) === String(cur)));
    const btns = stops.map((s, j) => `
      <button type="button" role="radio" aria-checked="${j === k}"
        class="oaa-rail-stop${j === k ? ' on' : ''}"${s.title ? ` title="${escapeHtml(s.title)}"` : ''}
        onclick="oaaSetParam('${name}','${s.v}')">${s.l}</button>`).join('');
    return `<div class="oaa-rail" role="radiogroup" aria-label="${escapeHtml(label)}">
      <span class="oaa-rail-l">${label}</span>
      <div class="oaa-rail-track" style="--n:${stops.length};--k:${k}">
        <span class="oaa-rail-fill" aria-hidden="true"></span>${btns}
      </div>
    </div>`;
  };
  const buckets = doc.docBuckets || [];
  return `<div class="oaa-params">
    ${rail('Camp', 'side', [
      { v: 'all', l: 'Les deux' },
      { v: 'w', l: '♔ Blancs', title: 'Erreurs commises par les Blancs' },
      { v: 'b', l: '♚ Noirs', title: 'Erreurs commises par les Noirs' },
    ], OA.side)}
    ${rail('Tranche', 'bucket', [
      { v: 'all', l: 'Toutes' },
      ...buckets.map(b => ({ v: b, l: bucketShort(b), title: bucketLabel(b, doc.fide) })),
    ], OA.bucket)}
    ${rail('Coût', 'minLoss', [
      { v: 0, l: 'Tous' },
      { v: 25, l: '≥ ¼' }, { v: 50, l: '≥ ½' },
      { v: 100, l: '≥ 1' }, { v: 200, l: '≥ 2' },
    ].map(s => ({ ...s, title: s.v ? `Coût d'au moins ${s.v / 100} pion${s.v >= 200 ? 's' : ''}` : 'Tous les coûts' })), OA.minLoss)}
    ${rail('Criticité', 'topCrit', [
      { v: 0, l: 'Toutes' },
      { v: 50, l: 'Top 50' }, { v: 20, l: 'Top 20' }, { v: 10, l: 'Top 10' },
    ].map(s => ({ ...s, title: s.v ? `Les ${s.v} erreurs les plus critiques du filtre` : 'Toutes les criticités' })), OA.topCrit)}
    <span class="oaa-params-count" aria-live="polite">${shown} / ${total} erreur${total > 1 ? 's' : ''}</span>
  </div>`;
}

// Le pipeline de filtre du panneau, partagé par la table et le compteur du
// toggle « mes élèves » (les deux doivent voir le MÊME sous-ensemble).
function _oaaFiltered(doc) {
  const base = filterErrorsIndexed(doc.errors, OA.bucket === 'all' ? 'all' : Number(OA.bucket),
    { side: OA.side, minLossCp: OA.minLoss });
  return capTopCrit(base, OA.topCrit);
}

function _oaaErrorsHTML(doc) {
  let errs = _oaaFiltered(doc);
  const shown = errs.length, total = (doc.errors || []).length;
  ...
  const mineCount = _oaaFiltered(doc)
  ...
  if (!errs.length) {
    return _oaaErrorsShellHTML(doc, mineCount,
      `<div class="oaa-none">${OA.onlyMine
        ? 'Aucune de ces erreurs n’a encore été ratée par tes élèves avec ces réglages.'
        : 'Aucune erreur ne passe ces réglages — élargis un rail (camp, tranche, coût…).'}</div>`,
      shown, total);
  }
  ...
  return _oaaErrorsShellHTML(doc, mineCount, table, shown, total);
}

// Coquille commune (panneau de paramètres + barre d'actions + grille table/aperçu)
// — partagée par l'état plein et les états vides pour que les réglages restent
// atteignables (c'est par eux qu'on sort d'un filtre trop étroit).
function _oaaErrorsShellHTML(doc, mineCount, inner, shown, total) {
  return `
    ${_oaaParamsHTML(doc, shown, total)}
    <div class="oaa-actions">
      ...
      <span id="oaa-selcount" class="oaa-selcount">${OA.sel.size ? OA.sel.size + ' sélectionnée' + (OA.sel.size > 1 ? 's' : '') : ''}</span>
...
// Chips de tranche des onglets Trous / Diagnostics : la tranche partagée
// (OA.bucket) a quitté la barre pour le rail « Tranche » de l'onglet Erreurs —
// ces onglets, qui la consomment aussi, portent désormais leur propre contrôle.
function _oaaBucketChipsHTML(doc, { concrete = false } = {}) {
  const buckets = doc.docBuckets || [];
  const effective = concrete && OA.bucket === 'all' ? String(buckets[0] ?? '') : String(OA.bucket);
  const all = concrete ? [] : ['all'];
  return `<span class="oaa-chips" role="group" aria-label="Tranche Elo">
    ${[...all, ...buckets].map(b => {
      const on = effective === String(b);
      return `<button type="button" class="oaa-chip${on ? ' on' : ''}" aria-pressed="${on}"
...
function _oaaGapsHTML(doc) {
  ...
  // Les trous n'existent que PAR tranche : les chips sont « concrètes » (pas de
  // « Toutes »), la tranche effective reste celle du rail quand elle est précise.
  const head = `<div class="oaa-gaps-head">${toggle}
    ${_oaaBucketChipsHTML(doc, { concrete: true })}</div>`;
...
function _oaaDiagHTML(doc) {
  const bucket = OA.bucket === 'all' ? String((doc.docBuckets || [1600])[0]) : String(OA.bucket);
  const chips = `<div class="oaa-gaps-head">${_oaaBucketChipsHTML(doc, { concrete: true })}</div>`;
...
// Rails du panneau de paramètres. Même règle que la tranche : tout changement
// vide la sélection (une erreur cochée hors filtre serait invisible et
// indécochable) et libère l'épingle ; le tout est mémorisé (mc_oaa_params).
function oaaSetParam(name, v) {
  if (name === 'bucket') { oaaSetBucket(v); return; }
  if (name === 'side') OA.side = (v === 'w' || v === 'b') ? v : 'all';
  else if (name === 'minLoss') OA.minLoss = Number(v) || 0;
  else if (name === 'topCrit') OA.topCrit = Number(v) || 0;
  OA.sel.clear(); OA.pin = null;
  try {
    localStorage.setItem('mc_oaa_params',
      JSON.stringify({ side: OA.side, minLoss: OA.minLoss, topCrit: OA.topCrit }));
  } catch (e) {}
  _oaaRerenderContent();
}
...
function oaaToggleSel(i, on) {
  ...
  if (el) el.textContent = OA.sel.size ? `${OA.sel.size} sélectionnée${OA.sel.size > 1 ? 's' : ''}` : '';
}
...
function oaaAddToSel(i) {
  ...
  const el = document.getElementById('oaa-selcount');
  if (el) el.textContent = `${OA.sel.size} sélectionnée${OA.sel.size > 1 ? 's' : ''}`;
...
Object.assign(window, {
  renderOaAnalytics, oaaSelectModule, oaaSetBucket, oaaSetParam, oaaSetTab, oaaSetGapColor,
