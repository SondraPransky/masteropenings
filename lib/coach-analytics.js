...
function _oaaErrorsHTML(doc) {
  ...
  const rows = errs.map(({ e, i }) => {
    ...
    return `<tr class="oaa-row${OA.pin === i ? ' pinned' : ''}" data-i="${i}"
      ...
      <td class="oaa-stu">${chips || '<span class="oaa-sub-td">—</span>'}</td>
  ...
  const table = `
    <div class="oaa-scroll"><table class="oaa-table wsx-table">
      <thead><tr><th></th><th>Ligne</th>
        ...
        <th>Meilleur</th>
        ${th('lossCp', 'Coût')}
        <th>Tes élèves</th><th>Tranche</th>
...
function _oaaPinHTML(doc) {
  ...
  if (!e) return `<div class="oaa-pin-empty">Survole une ligne pour voir la position.</div>`;
  const flip = e.stm === 'b';
  const arrows = _oaaArrowsSvg([[e.uci, 'var(--red)'], [e.bestUci, 'var(--green)']], flip);
  return `
...
// Calque de flèches en pourcentage (le plateau miniboard est une grille 8×8 fluide).
function _oaaArrowsSvg(pairs, flip) {
  const sq = uci => {
    ...
    return [(f + .5) * 12.5, (r + .5) * 12.5, (f2 + .5) * 12.5, (r2 + .5) * 12.5];
  };
  const lines = pairs.map(([uci, color], k) => {
    ...
    return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="2.2"
      stroke-linecap="round" marker-end="url(#oaa-ah${k})" opacity=".85"/>`;
  }).join('');
  const markers = pairs.map(([, color], k) =>
    `<marker id="oaa-ah${k}" markerWidth="4" markerHeight="4" refX="2.6" refY="2" orient="auto">
      <path d="M0,0 L4,2 L0,4 z" fill="${color}"/></marker>`).join('');
  return `<svg class="oaa-arrows" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
  ...
  box.innerHTML = `
    <div class="oaa-pos-grid">
      <div class="oaa-pin-wrap oaa-pos-board">${renderStaticBoard(e.fen, { size: 320, flip })}${
        _oaaArrowsSvg([[e.uci, 'var(--red)'], [e.bestUci, 'var(--green)']], flip)}</div>
      <div>
...
Object.assign(window, {
  ...
  oaaToggleSel, oaaRefresh, oaaSort, oaaToggleMine, oaaHover, oaaPin, oaaLichessNote,
