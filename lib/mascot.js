...
// `pose` : 'wave' (défaut) et 'cheer' gardent les yeux ouverts ; 'sleep' ferme les
// paupières (arc de pixels) et saute les reflets blancs.
function build(th, pose){
  const g=grid();
  ...
  // Paupières fermées (pose sleep) : un arc doux, pas d'amande ni de reflet.
  const lidAt=(cx)=>{ [[cx-2,18],[cx+2,18],[cx-1,19],[cx,19],[cx+1,19]].forEach(p=>set(g,p[0],p[1],'k')); };
  if (pose==='sleep'){ lidAt(15); lidAt(25); } else { eyeAt(15,-1); eyeAt(25,1); }
...
// « Z z » pixel de la pose sleep — calque animé en CSS (fondu), couleur crête indigo.
function zzRects(P){
  const px=(x,y,s)=>`<rect x="${x}" y="${y}" width="${s}" height="${s}" fill="${P.i}"/>`;
  const z=(ox,oy,s)=>[[0,0],[1,0],[2,0],[1,1],[0,2],[1,2],[2,2]].map(p=>px(ox+p[0]*s,oy+p[1]*s,s)).join('');
  return `<g class="msc-z1">${z(32,7,1)}</g><g class="msc-z2">${z(35,2,1.4)}</g>`;
}

/** SVG de la mascotte (robe active). `size` = largeur px (hauteur dérivée) ;
 *  `pose` = 'wave' (salut, défaut) · 'cheer' (fête, fin sans-faute) · 'sleep'
 *  (sieste, états vides : paupières closes, pas de patte levée, « Z z »). */
export function mascotSvg(size = 80, pose = 'wave') {
  const th = PALETTES[ACTIVE], P = pal(th);
  const paw = pose === 'sleep' ? '' : `<g class="msc-paw">${rects(pawGrid(), P)}</g>`;
  const zz  = pose === 'sleep' ? zzRects(P) : '';
  const label = pose === 'sleep' ? 'Mascotte chat-cavalier qui dort' : 'Mascotte chat-cavalier';
  return `<svg class="msc msc-${pose}" width="${size}" height="${Math.round(size * H / W)}" viewBox="0 0 ${W} ${H}" `
    + `shape-rendering="crispEdges" role="img" aria-label="${label}">`
    + `<g class="msc-anim"><g class="msc-tail">${rects(tailGrid(th.stripes), P)}</g>`
    + `${rects(build(th, pose), P)}${paw}${zz}</g></svg>`;
}
