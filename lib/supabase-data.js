...
async function _coachLoad() {
  ...
  await _sbLoadStudentPhotos();     // photos posees par le coach (profiles.extra.studentPhotos)
  ...
  window.startWeaknessQueue?.(); // assistant faiblesses : analyse moteur des parties partagées pas encore traitées
}
...
// Parties de l'élève connecté (sa bibliothèque) → multi-appareils.
...
async function _sbLoadTeacherGames() {
  return _sbRun('_sbLoadTeacherGames', sb && G.currentUser && G.currentRole === 'teacher', async () => {
    // Parties bibliothèque partagées (drill_id null) — RLS games_read filtre aux élèves du prof.
    const { data: libRows } = await sb.from('games').select('*').is('drill_id', null);
    G.savedGames = (libRows || []).map(_sbRowToGame).filter(g => g.shared);
    _cache('mc_games', G.savedGames);
  });
}

// Analyses OA (worker Supabase D18, migration-007) — lues à la demande par la
// section coach « Analyse d'ouvertures ». Une ligne par module ; le doc `data`
// est déjà compact et précalculé (le worker `py -m oa.eecoach_worker` le remplit).
// Cache de session : les docs pèsent 100-200 Ko chacun — on ne les re-télécharge
// pas à chaque ouverture de la section. `force` (bouton Actualiser) recharge.
let _oaLoaded = false;
async function _sbLoadOaAnalyses(force) {
  if (_oaLoaded && !force) return;
  return _sbRun('_sbLoadOaAnalyses', sb && G.currentUser && G.currentRole === 'teacher', async () => {
    const { data } = await sb.from('oa_analyses').select('module_id,updated_at,data');
    const byId = {};
    (data || []).forEach(r => { byId[r.module_id] = { updatedAt: r.updated_at, data: r.data || {} }; });
    G.oaAnalyses = byId;
    _oaLoaded = true;
  });
}
...
// ── Photos des élèves (annotation du coach) — profiles.extra.studentPhotos ──
// ⚠ Le profil du coach porte AUSSI d'autres clés d'extra (ex. parentLinks) : on
// FUSIONNE lecture-modification-écriture, jamais un update naïf qui les écraserait
// (le piège documenté de _sbSaveBases, ici évité côté coach).
async function _sbSaveStudentPhotos() {
  return _sbRun('_sbSaveStudentPhotos', sb && G.currentUser && G.currentRole === 'teacher', async () => {
    const { data } = await sb.from('profiles').select('extra').eq('id', G.currentUser.uid).maybeSingle();
    const extra = { ...(data && data.extra ? data.extra : {}), studentPhotos: G.studentPhotos };
    const { error } = await sb.from('profiles').update({ extra }).eq('id', G.currentUser.uid);
    if (error) throw error;
  });
}

async function _sbLoadStudentPhotos() {
  if (!sb || !G.currentUser) return;
  try {
    const { data } = await sb.from('profiles').select('extra').eq('id', G.currentUser.uid).maybeSingle();
    G.studentPhotos = (data && data.extra && data.extra.studentPhotos) || {};
    _cache('mc_student_photos', G.studentPhotos);
    // Synthèses de l'assistant faiblesses (même ligne de profil, chargées au passage).
    G.studentBriefs = (data && data.extra && data.extra.studentBriefs) || {};
    _cache('mc_student_briefs', G.studentBriefs);
  } catch (e) { console.warn('_sbLoadStudentPhotos (colonne extra manquante ?)', e); }
}

// Synthèses rédigées de l'assistant faiblesses ({ cléÉlève: {ts, text} }) — dans le
// profil du COACH, lecture-fusion-écriture (⚠ un update naïf d'`extra` écraserait
// studentPhotos/parentLinks — le piège documenté de _sbSaveBases).
async function _sbSaveStudentBriefs() {
  return _sbRun('_sbSaveStudentBriefs', sb && G.currentUser && G.currentRole === 'teacher', async () => {
    const { data } = await sb.from('profiles').select('extra').eq('id', G.currentUser.uid).maybeSingle();
    const extra = { ...(data && data.extra ? data.extra : {}), studentBriefs: G.studentBriefs };
    const { error } = await sb.from('profiles').update({ extra }).eq('id', G.currentUser.uid);
    if (error) throw error;
  });
}
...
Object.assign(window, {
  ...
  _sbLoadTeacherOverlays, _sbSaveCoachOverlayReply, _sbLoadOaAnalyses,
  _sbSaveStudentPhotos, _sbSaveStudentBriefs,
});
