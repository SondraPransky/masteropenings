...
// `fig` s'importe de core.js (fonction pure) : le shim window rendait du SAN brut
// quand un rendu partait avant la pose du pont (même bug que renderDrillList, 22/07).
export { fig, moveLabel } from './core.js';   // moveLabel : « 12. » / « 12… » (une seule forme pour toutes les vues)
...
// ── Avatar d'un élève : photo (posée par le coach) ou pastille à initiale ──
// Clé stable = identité de l'élève (s.key en liste / ctx.id en détail), en minuscules.
export const _photoKey = k => String(k || '').trim().toLowerCase();
export function _studentAvatar(key, label, size = 32) {
  const url = (G.studentPhotos || {})[_photoKey(key)];
  const s = `width:${size}px;height:${size}px`;
  if (url) return `<img class="stu-avatar" src="${escapeHtml(url)}" alt="" style="${s}">`;
  const init = (String(label || key || '?').trim()[0] || '?').toUpperCase();
  return `<span class="stu-avatar stu-avatar-init" aria-hidden="true" style="${s};font-size:${Math.round(size*.42)}px">${escapeHtml(init)}</span>`;
}
