...
function _exRenderList() {
  ...
  el.innerHTML = _EX.exercises.map((k, i) => {
    ...
    return `<div class="ex-row" style="display:flex;gap:12px;align-items:center;padding:10px;border:1px solid var(--border);border-radius:var(--r);margin-bottom:8px">
...
// LE constructeur canonique du module-paquet : la seule source du schéma
// (revue 21/07 — il existait en 3 copies : ici, explorateur OTKB, analyse OA).
// Les autres modules l'appellent via le pont window (règle coach-*).
function buildExercisePacket({ name, kps, level = 'Intermédiaire', exType = null, existing = null }) {
...
function saveExercisePacket() {
  ...
  // Orientation du plateau (positions mode : S.flipped = side==='b') = trait du
  // 1er exercice — calculée dans le constructeur canonique.
  const mod = buildExercisePacket({ name, kps, level, exType, existing });
  if (existing) {
...
Object.assign(window, {
  ...
  exDeleteExercise, _exSolClick, exSolReset, exSolUndo, exSolAdd, saveExercisePacket, buildExercisePacket,
