...
//    tryMove/canInteract/showPromoPicker…) : les ~56 appelants (drill, study,
//    sr, app) sont INCHANGÉS. Seul le CORPS change : piloter une instance
...
// - Entrée : `movable.events.after(orig,dest)` → `tryMove(orig,dest)` (dispatch
//   INCHANGÉ).
...
// REPEINT PAS le SVG des formes (mesuré : 0 forme rendue, 1 après `redrawAll`).
// Bug latent de l'étape B : les flèches/cercles d'étude ne se sont JAMAIS affichés
// depuis la bascule canvas → Chessground. On repeint SEULEMENT quand les formes
// changent (garde de signature) pour ne pas payer un redrawAll à chaque coup.
let _lastShapesSig = '';
function _setAutoShapes(shapes) {
  _cg.setAutoShapes(shapes);
  const sig = shapes.map(s => s.orig + '>' + (s.dest || '') + ':' + s.brush).join('|');
  if (sig !== _lastShapesSig) { _lastShapesSig = sig; _cg.redrawAll(); }
}
...
function drawBoard() {
  ...
  // Indice (case source dorée) + flèche de la carte flash + flèches/cercles du
  // PGN en phase étude.
  const shapes = [];
  if (S.hintSquare) shapes.push({ orig: S.hintSquare, brush: 'yellow' });
  if (S.flashArrow) shapes.push({ orig: S.flashArrow.from, dest: S.flashArrow.to, brush: 'green' });
...
function canInteract() {
  ...
  const g=currentGame();
  if(!g) return false;
  // Session SR : flux « positions » (S.kps/S.posIdx), quel que soit le mode PROPRE
  // du module — même précédence que tryMove et currentGame. Sans cette garde, un
  // module `varmode:'tree'` retombait sur S.waitingForPlayer (jamais posé en SR)
  // et le plateau restait en LECTURE SEULE pendant toute la révision.
  if(S.sr && S.sr.active) return S.posIdx < S.kps.length;
...
function tryMove(from, to) {
  ...
  if(S.drill?.varmode==='tree') window.tryMoveInTree?.(from,to);
...
// ── Promotion picker (INCHANGÉ — partagé avec l'éditeur et l'explorateur) ──────
