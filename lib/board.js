...
// Couleurs de cases lues depuis les variables CSS (source unique, pilotée par le
// sélecteur de thème [data-board]) — une lecture par rendu, mise en cache.
function _boardColors() {
  const cs = getComputedStyle(document.documentElement);
  const light = cs.getPropertyValue('--board-light').trim() || '#f0d9b5';
  const dark  = cs.getPropertyValue('--board-dark').trim()  || '#b58863';
  return { light, dark };
}
...
function drawBoard() {
  ...
  for(let row=0;row<8;row++){
    for(let col=0;col<8;col++){
      ...
      const bg=light?COL.light:COL.dark;
