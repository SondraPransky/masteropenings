...
// ── Reordonner les chapitres d'un module (fleches haut/bas) ─────────────────
// On echange DEUX parties du PGN (la source de verite) puis on rebatit : l'ordre
// des sessions suit l'ordre du PGN (buildTreeModule reassigne les gameIdx).
// `chapTitles` est cle par gameIdx → le titre edite suit le contenu deplace :
// on echange t[gi] <-> t[gj], sinon il resterait colle a l'ancienne position.
function reorderChapter(id, k, dir) {
  const d = _mod(id);
  if (!d) return;
  const n = d.sessions?.length || 0;
  const j = k + dir;
  if (j < 0 || j >= n) return;                       // deja a une extremite
  const gi = _gameIdx(d, k), gj = _gameIdx(d, j);
  const games = splitPgnGames(d.pgn || '');
  if (games[gi] == null || games[gj] == null || gi === gj) return;
  [games[gi], games[gj]] = [games[gj], games[gi]];   // echange le contenu des deux parties
  const newPgn = games.filter(Boolean).join('\n\n');
  const t = { ...(d.chapTitles || {}) };
  const ti = t[String(gi)], tj = t[String(gj)];      // le titre suit son contenu
  if (tj === undefined) delete t[String(gi)]; else t[String(gi)] = tj;
  if (ti === undefined) delete t[String(gj)]; else t[String(gj)] = ti;
  const snap = { pgn: d.pgn, tree: d.tree, sessions: d.sessions, chapTitles: d.chapTitles, updatedAt: d.updatedAt };
  if (!_rebuild(d, newPgn, t)) {
    Object.assign(d, snap);                           // rien ne bouge si le rebuild echoue
    toast('❌ Reordonnancement impossible — rien n\'a change', 'ko');
    return;
  }
  _persist(d);                                        // renderDrillList garde le module ouvert (_modOpenId)
  toast('✓ Ordre des chapitres mis a jour', 'ok');
}

Object.assign(window, {
  openChapterTransfer, chapterTransferMode, chapterTransferRun,
  openChapterPaste, chapterPasteRun, reorderChapter,
});

export { openChapterTransfer, chapterTransferRun, openChapterPaste, chapterPasteRun, reorderChapter };
