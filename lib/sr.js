...
import { _treePositionsScan, _materialHint } from './tree.js';
...
// ── « Ma séance du jour » (arbitrage utilisatrice 28/07) ────────────────────
// La routine COURTE qui retient les enfants (recherche : 10-15 min réguliers
// battent les longues sessions) : recette FIXE, zéro réglage, un seul bouton.
// ~10 min = plafond de 16 cartes : les révisions les plus en retard d'abord,
// avec une place garantie pour quelques nouveautés (4) si la file en offre.
// La file 'all' couvre déjà modules + paquets d'exercices + membres de plans,
// donc la séance est naturellement mixte — rien de nouveau à indexer.
const _SR_DAILY_CAP = 16;

function _srDailyPlan() {
  const student = S.student || (G.currentUser ? (G.currentUser.displayName || G.currentUser.email) : '');
  if (!student) return { queue: [], rev: 0, nw: 0, more: 0 };
  const { queue } = _srBuildQueue(_srScopeList('all'), student);
  const revs = queue.filter(c => !c._srNew).sort((a, b) => (a._due || 0) - (b._due || 0));
  const news = queue.filter(c => c._srNew);
  const newSlots = Math.min(4, news.length);
  const revPick = revs.slice(0, _SR_DAILY_CAP - newSlots);
  const newPick = news.slice(0, _SR_DAILY_CAP - revPick.length);
  const combined = revPick.concat(newPick);
  // Même politique d'ordre que la session classique (mixed = mélangé).
  if ((localStorage.getItem('mc_sr_order') || 'mixed') !== 'due') {
    for (let k = combined.length - 1; k > 0; k--) { const j = Math.floor(Math.random() * (k + 1)); [combined[k], combined[j]] = [combined[j], combined[k]]; }
  }
  return { queue: combined, rev: revPick.length, nw: newPick.length, more: queue.length - combined.length };
}

function srStartDaily() {
  const plan = _srDailyPlan();
  if (!plan.queue.length) { toast('✓ Rien à réviser pour le moment !', 'ok'); return; }
  srStart('all', undefined, plan);
}
...
function _srPositions(d) {
  if (!d) return [];
  // Balayage O(n) mémoïsé, PAS la BFS : ce chemin est appelé sur TOUS les modules
  // à chaque rendu des compteurs du hero (via _srSessionSize('all')) — la BFS y
  // coûtait ~23 s sur le corpus réel. Même ensemble, mesuré (cf. lib/tree.js).
  if (d.varmode === 'tree') return _treePositionsScan(d);
  const out = [];
...
// `dailyPlan` (optionnel, via srStartDaily) : file déjà composée et plafonnée —
// la session devient « Ma séance du jour » (titre, bilan célébré, toasts adaptés).
function srStart(scope, drillIdx, dailyPlan) {
  if (!S.student) { toast('⚠ Identifiez-vous d\'abord', 'ko'); return; }
  const student = S.student;
  let { queue, revTotal, newTotal, revBacklog } = _srBuildQueue(_srScopeList(scope, drillIdx), student);
  if (dailyPlan) { queue = dailyPlan.queue; revTotal = dailyPlan.rev; newTotal = dailyPlan.nw; }
  ...
  S.ok = 0; S.ko = 0; S.sel = null; S.sessionIdx = 0; S.chapterTree = null;
  S.phase = 'test'; S._reviewMode = true; S.unifiedReview = (scope !== 'drill');
  S.kps = queue;
  document.getElementById('s-name').textContent  = dailyPlan ? 'Ma séance du jour'
                                                 : scope === 'drill' ? (S.drill.name + ' — Révision') : '↻ Révision espacée';
  ...
  // ⚠️ RÉVÉLER LA PAGE AVANT de dessiner (patron de startStudentDrill) :
  ...
  // Séance du jour : le plafond est LE contrat (~10 min), pas un retard — un seul
  // message calme, jamais le bandeau backlog anxiogène.
  if (dailyPlan) {
    if (dailyPlan.more > 0) toast(`Une séance courte aujourd'hui — ${dailyPlan.more} autre${dailyPlan.more > 1 ? 's' : ''} position${dailyPlan.more > 1 ? 's' : ''} attendr${dailyPlan.more > 1 ? 'ont' : 'a'} la prochaine.`, 'ok');
  } else {
...
function _srAnswer(kp, played, isCorrect) {
  ...
  if (isCorrect) {
    ...
    // `_reteach` (+ `_taught` remis à false) : la copie requeue repasse par la
    // carte flash avant d'être re-testée — on ré-enseigne, on ne re-teste pas à
    // froid 3 positions plus loin. Sans effet sur Leitner (seule la 1re tentative
    // pilote la planification, `S.sr.graded`).
    S.kps.splice(insertAt, 0, { ...kp, attempted: false, correct: false, _requeued: true, _reteach: true, _taught: false });
    drawBoard(); _srUpdateBar();
    // Correction au format carte (B3) : bouton « Continuer » au lieu de la
    // minuterie de 1400 ms — l'élève lit la correction à son rythme.
    if (window._flashCorrection) window._flashCorrection(kp, { hint: matHint, next: true, requeued: true });
    else {
...
function _srBilan() {
  ...
  if (title) title.textContent = daily ? 'Séance du jour terminée !' : '✓ Session terminée';
  // Séance du jour : la routine courte MÉRITE sa célébration (mascotte cheer,
  // le même budget délice que le sans-faute d'exercices) — c'est la fin de
  // séance qui construit l'envie de revenir demain.
  const cheer = daily
    ? `<div style="display:flex;align-items:center;justify-content:center;gap:12px;margin:-4px 0 12px">${mascotSvg(56, 'cheer')}<div style="font-size:.86rem;color:var(--text-2)">C'est fait pour aujourd'hui — reviens demain pour ta série.</div></div>`
    : '';
  document.getElementById('end-body').innerHTML = cheer
    ...
    // « Rétention » est le dernier jargon SR visible par l'élève : le tableau de bord
    // l'avait déjà remplacé par « réussite », pas ce bilan de fin de session.
    + `<div class="score-box"><div class="score-val" style="color:var(--green)">${retention}%</div><div class="score-lbl">Réussite</div></div>`
...
Object.assign(window, {
  updateReviserToutBadge, reviserTout, reviserDrill, srStartDaily, _srDailyPlan, _srNewLimit, _srTodayKey, _srNewToday,
