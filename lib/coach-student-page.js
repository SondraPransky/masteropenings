...
function _gamesHTML(ctx) {
  ...
  const maia   = ctx.games.filter(g => !g.baseId).sort((a, b) => (b.ts || 0) - (a.ts || 0));
  ...
  return `<section class="sp-sec">
    ...
    ${shared.length ? _capList(rowShared, 4, 'partie') : '<div class="sp-empty">Aucune partie partagée.</div>'}
    ${maia.length ? `<div class="sp-subhead">Parties contre Maia</div>${_capList(rowMaia, 3, 'partie')}` : ''}
  </section>`;
}
