...
function _gamesHTML(ctx) {
  ...
  const rowShared = shared.map(g => `<div class="sp-row">
    ...
    <div class="sp-row-tail">
      ...
      <button class="btn btn-blue btn-sm" onclick="annotateSharedGame('${escapeHtml(String(g.id))}')"><i class="ti ti-eye" aria-hidden="true"></i> Voir</button>
    </div>
  </div>`);
...
Object.assign(window, { renderStudentPage, spPickPhoto, spRemovePhoto });
