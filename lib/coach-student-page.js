...
import { weaknessReport, briefPayload } from './weakness-core.js';
...
import {
  ...
  _studentIdSet, _studentAvatar, _photoKey, sm2Get, fig, figText, moveLabel, escapeHtml, toast,
} from './coach-core.js';
...
function _weakHTML(ctx) {
  ...
  const exitRows = exits.slice(0, 6).map(e => {
    const num = moveLabel(e.moveNo, e.color);
    // data-fen : la position EXACTE de la sortie. Sans elle, assignReviewForStudent
    // retombe sur _drillFenMap (san → fen, premier arrivé sur tout l'arbre) et
    // assignerait la révision sur une autre position portant le même SAN.
    return `<div class="ed-review-item" tabindex="0" data-did="${escapeHtml(String(e.drillId))}" data-san="${escapeHtml(e.expected[0] || '')}" data-fen="${escapeHtml(e.fenBefore)}">
  ...
  // — Fautes tactiques : `faults.all` arrive déjà trié par coût décroissant.
  const faultRows = faults.all.slice(0, 6).map(ft => {
    const num = moveLabel(ft.moveNo, ft.color);
  ...
  const parPhase = ['ouverture', 'milieu', 'finale']
    .map(p => f.byPhase[p] ? `${f.byPhase[p]} en ${p === 'milieu' ? 'milieu de jeu' : p}` : null)
    .filter(Boolean).join(' · ');
...
async function spGenBrief() {
  ...
  const label = btn && btn.innerHTML;   // « Générer » ou « Régénérer » selon l'état
  if (btn) { btn.disabled = true; btn.innerHTML = '<i class="ti ti-loader-2 wq-spin" aria-hidden="true"></i> Rédaction…'; }
  try {
    const payload = briefPayload(report, ctx.display);
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = label; }   // « Régénérer » ne redevient pas « Générer »
  }
}
...
function _positionsHTML(ctx) {
  ...
  const groups = Object.entries(byDrill).map(([dn, dd]) => {
    // ⚠ `posIdx` n'existe PAS sur les résultats venus d'une session de répétition
    // espacée : ni `_srPositions` ni `_treePositionsScan` ne le stockent, et la
    // séance du jour EST une session SR — donc c'est le cas le PLUS courant. Le tri
    // range les positions sans index en fin de liste plutôt que de comparer des NaN.
    const rang = (p) => Number.isFinite(p.posIdx) ? p.posIdx : Number.MAX_SAFE_INTEGER;
    const arr = Object.entries(dd.positions).sort((a, b) => rang(a[1]) - rang(b[1]))
      ...
      <table class="pos-table">
        ...
        <tbody>${arr.map((p, i) => {
          ...
          return `<tr tabindex="0" data-did="${escapeHtml(String(dd.id))}" data-san="${escapeHtml(p.san || '')}"
            onmouseenter="wsTip(event,this.dataset.did,this.dataset.san)" onmouseleave="wsTipHide()"
            onfocus="wsTip(event,this.dataset.did,this.dataset.san)" onblur="wsTipHide()">
            <td class="ed-pos-num">${Number.isFinite(p.posIdx) ? p.posIdx + 1 : i + 1}</td>
