  ...
  return groups.map(gr => `<div class="pg-stu-sec">
    ...
    ${gr.arr.map(g => {
      ...
      const statut = g.reviewedAt
        ? `<span class="pg-state pg-state-done" title="Tu l'as annotée"><i class="ti ti-sparkles" aria-hidden="true"></i> Annotée</span>`
      ...
      return `<div class="pg-row${g.reviewedAt ? '' : ' pg-row-todo'}">
        <span class="game-result ${g.result==='1-0'?'win':g.result==='0-1'?'loss':'draw'}" title="Résultat">${g.result||'*'}</span>
        <button class="pg-row-open" onclick="annotateSharedGame('${escapeHtml(String(g.id))}')">
          <span class="pg-row-who">${escapeHtml(adv)}</span>
          <span class="pg-row-meta">${escapeHtml(cote)} · ${escapeHtml(g.event||'—')} · ${dt}${g.nature==='analyse'?' · Analyse':''}</span>
        </button>
        ${statut}
        ${_pgRepHTML(g)}
      </div>`;
