    ...
    ${gr.arr.map(g => {
      ...
      return `<div class="game-row">
        <div class="game-row-main">
          ...
          <button class="btn ${g.reviewedAt?'btn-ghost':'btn-primary'} btn-sm" onclick="annotateSharedGame('${escapeHtml(String(g.id))}')"><i class="ti ${g.reviewedAt?'ti-book':'ti-eye'}" aria-hidden="true"></i> Ouvrir</button>
        </div>
        ${_pgRepHTML(g)}
      </div>`;
    }).join('')}
