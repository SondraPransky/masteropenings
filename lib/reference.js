...
// ── Parties de référence : TABLE à colonnes, TOUTES triables (demande 03/08) ────
// Clic sur un en-tête = tri décroissant (ou A→Z pour les noms), re-clic = inverse.
// Remis à zéro à chaque nouvelle position (l'ordre Lichess redevient le défaut).
let _refGSort = null;   // { key, dir: -1|1 } | null
// Valeur de tri par colonne. Résultat : 1-0 > ½-½ > 0-1 (le point de vue Blancs).
const _refGVal = {
  wn: g => g.white?.name || '',   we: g => g.white?.rating || 0,
  bn: g => g.black?.name || '',   be: g => g.black?.rating || 0,
  yr: g => g.year || 0,           res: g => g.winner === 'white' ? 2 : g.winner === 'black' ? 0 : 1,
};
const _REF_G_TITLES = { wn: 'le nom des Blancs', we: "l'Elo des Blancs", bn: 'le nom des Noirs',
                        be: "l'Elo des Noirs", yr: "l'année", res: 'le résultat' };
function _refSortGames(key) {
  _refGSort = (_refGSort && _refGSort.key === key)
    ? { key, dir: -_refGSort.dir }
    : { key, dir: (key === 'wn' || key === 'bn') ? 1 : -1 };   // noms : A→Z d'abord
...
function _gamesHTML(topGames) {
  ...
  let games = topGames.slice();
  if (_refGSort) {
    const v = _refGVal[_refGSort.key];
    games.sort((a, b) => {
      const x = v(a), y = v(b);
      const c = typeof x === 'string' ? x.localeCompare(y, 'fr') : x - y;
      return c * _refGSort.dir;
    });
  }
  const sortBtn = (key, label) => {
    const on = _refGSort && _refGSort.key === key;
    const dir = on ? (_refGSort.dir === -1 ? 'descending' : 'ascending') : 'none';
    const ico = on ? `<i class="ti ti-chevron-${_refGSort.dir === -1 ? 'down' : 'up'}" aria-hidden="true"></i>` : '';
    return `<button type="button" class="ref-gt-sort${on ? ' on' : ''}" aria-sort="${dir}"
      title="Trier par ${_REF_G_TITLES[key]}" onclick="_refSortGames('${key}')">${label}${ico}</button>`;
  };
  const rows = games.map(g => {
    const wn = g.white?.name || '?', bn = g.black?.name || '?';
    const we = g.white?.rating || '', be = g.black?.rating || '';
    const attrs = g.id
      ? ` class="ref-gt-row" tabindex="0" data-id="${escapeHtml(g.id)}" title="Ouvrir la partie ici"
          onclick="_refClickGame(this.dataset.id)"
          onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();_refClickGame(this.dataset.id)}"`
      : ' class="ref-gt-row off"';
    return `<tr${attrs}>
      <td class="ref-gt-name">${escapeHtml(wn)}</td><td class="ref-gt-elo">${we}</td>
      <td class="ref-gt-name">${escapeHtml(bn)}</td><td class="ref-gt-elo">${be}</td>
      <td class="ref-gt-yr">${g.year || ''}</td><td class="ref-gt-res">${_refResult(g.winner)}</td>
    </tr>`;
  }).join('');
  return `<div class="ref-games-head">Parties de référence</div>
    <table class="ref-gtable">
      <thead><tr><th scope="col">${sortBtn('wn', 'Blancs')}</th><th scope="col">${sortBtn('we', 'Elo')}</th>
        <th scope="col">${sortBtn('bn', 'Noirs')}</th><th scope="col">${sortBtn('be', 'Elo')}</th>
        <th scope="col">${sortBtn('yr', 'Année')}</th><th scope="col" title="Résultat">${sortBtn('res', 'Rés.')}</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
...
function _panelHTML(data) {
  ...
  if (!data.moves || !data.moves.length) {
    return `<div class="ref-head"><div class="ref-head-sub">${escapeHtml(opening)}${total ? ` · ${total.toLocaleString('fr-FR')} parties de maîtres` : ''}</div></div>
  ...
  const rows = data.moves.map(m => {
    ...
    // Une seule ligne par coup (03/08) : coup · N parties · % · barre — la barre
    // prend la place restante (façon explorateur Lichess), ~2× moins haut.
    return `<button type="button" class="ref-move" onclick="_refClickMove('${escapeHtml(m.san)}')" title="Jouer ${escapeHtml(m.san)}">
      <span class="mono-move ref-move-san">${fig(escapeHtml(m.san))}</span>
      <span class="ref-move-n">${g.toLocaleString('fr-FR')} · ${share}%</span>
      ${_wdlBar(m.white || 0, m.draws || 0, m.black || 0)}
    </button>`;
  }).join('');
  return `<div class="ref-head">
      <div class="ref-head-sub">${escapeHtml(opening)} · ${total.toLocaleString('fr-FR')} parties de maîtres</div>
    </div>
...
// (03/08) Le titre « Référence — Maîtres » vit désormais dans l'ONGLET de l'hôte
// (éditeur + lecture de partie) : le panneau n'est rendu que quand son onglet est
// actif — c'est l'hôte qui décide, et un onglet inactif ne coûte aucun appel réseau.
...
async function renderReferencePanel({ hostId, fen, onMove, onInsert, flip }) {
  ...
  _refGSort = null;                    // le tri des parties repart au défaut Lichess
...
Object.assign(window, {
  renderReferencePanel, _refClickMove, _refSortGames,
