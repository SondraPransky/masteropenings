...
// ── Parties de référence : TABLE à colonnes, triable par Elo (demande 03/08) ────
// Tri local au panneau : cliquer « Elo » (Blancs ou Noirs) trie décroissant, re-cliquer
// inverse. Remis à zéro à chaque nouvelle position (l'ordre Lichess redevient le défaut).
let _refGSort = null;   // { key: 'w'|'b', dir: -1|1 } | null
...
function _gamesHTML(topGames) {
  ...
  if (_refGSort) {
    const r = g => (_refGSort.key === 'w' ? g.white?.rating : g.black?.rating) || 0;
    games.sort((a, b) => (r(b) - r(a)) * (_refGSort.dir === -1 ? 1 : -1));
  }
  const sortBtn = (key, label) => {
    ...
    return `<button type="button" class="ref-gt-sort${on ? ' on' : ''}" aria-sort="${dir}"
      title="Trier par Elo des ${key === 'w' ? 'Blancs' : 'Noirs'}" onclick="_refSortGames('${key}')">${label}${ico}</button>`;
  };
  ...
  return `<div class="ref-games-head">Parties de référence</div>
    <table class="ref-gtable">
      <thead><tr><th scope="col">Blancs</th><th scope="col">${sortBtn('w', 'Elo')}</th>
        <th scope="col">Noirs</th><th scope="col">${sortBtn('b', 'Elo')}</th>
        <th scope="col">Année</th><th scope="col" title="Résultat">Rés.</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
