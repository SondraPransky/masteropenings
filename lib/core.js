...
// Insere une ou plusieurs parties dans un PGN multi-parties (a `atIdx`, ou a la
// fin si `atIdx` est null/hors bornes). Sert au transfert de chapitres d'un
// module a l'autre et au collage d'une partie dans un module existant.
// ⚠ splitPgnGames decoupe sur `[Event `. Un chunk SANS en-tete se collerait donc
// au chapitre precedent (une seule partie au lieu de deux) : on lui en pose un.
// Le BOM UTF-8 des exports ChessBase masque le [Event] de tete → retire ici,
// sinon il se retrouverait au milieu du PGN (piege du 22/07).
function insertPgnGames(pgn, chunkPgn, atIdx = null) {
  const clean = (chunkPgn || '').replace(/^﻿/, '').replace(/\r/g, '').trim();
  if (!clean) return pgn || '';
  const added = splitPgnGames(clean).map(c => /^\[Event\s/.test(c) ? c : `[Event "?"]\n\n${c}`);
  if (!added.length) return pgn || '';
  const games = splitPgnGames(pgn || '');
  const at = (atIdx == null || atIdx < 0 || atIdx > games.length) ? games.length : atIdx;
  games.splice(at, 0, ...added);
  return games.filter(Boolean).join('\n\n');
}

// Coups SAN de la LIGNE PRINCIPALE de la premiere partie d'un PGN, sans les
// rejouer (pas d'objet Chess : c'est un affichage, appele par liste entiere).
// Ignore commentaires {…}, variantes (…) a toute profondeur, NAG $n, numeros de
// coups et resultat. Sert a la « forme » de l'ouverture dans les listes de
// parties (l'idee de la maquette europe echecs : 4 demi-coups en figurines se
// reconnaissent d'un coup d'oeil, un code ECO non) et a l'index de positions.
function pgnMainlineSans(pgn, maxPlies = Infinity) {
  const game = splitPgnGames(pgn || '')[0] || pgn || '';
  const movetext = game.replace(/^\s*(?:\[[^\]]*\]\s*)+/, '');
  const clean = movetext.replace(/\{[^}]*\}/g, ' ').replace(/\(/g, ' ( ').replace(/\)/g, ' ) ');
  const sans = [];
  let depth = 0;
  for (let tok of clean.split(/\s+/)) {
    ...
    if (depth > 0) continue;                                  // variante : hors ligne principale
    tok = tok.replace(/^\d+\.{1,3}/, '');                     // « 12.Nf3 » colle le numero
    if (!tok || /^\$\d+$/.test(tok)) continue;                // NAG
    if (/^(1-0|0-1|1\/2-1\/2|\*)$/.test(tok)) break;          // resultat = fin de partie
    if (!/^[KQRBNOa-h0]/.test(tok)) continue;                 // pas une notation
    sans.push(tok.replace(/[!?]+$/, ''));                     // « Nf3!? » → « Nf3 » (le + / # restent)
    if (sans.length >= maxPlies) break;
  }
  return sans;
}

// Retire la partie `idx` d'un PGN multi-parties. Retourne '' s'il n'en restait
// qu'une (l'appelant doit alors refuser l'operation plutot que vider le module).
function removePgnGame(pgn, idx) {
  const games = splitPgnGames(pgn || '');
  if (idx < 0 || idx >= games.length) return pgn || '';
  games.splice(idx, 1);
  return games.filter(Boolean).join('\n\n');
...
// SAN français → anglais (« Cf3 » → « Nf3 », « e8=D » → « e8=Q ») pour la saisie
// au clavier depuis une feuille de partie (les enfants notent en français).
// ⚠ Ici, PAS d'ambiguïté de langue (contrairement aux titres du corpus, mixtes) :
// l'appelant déclare « cette saisie est française » et traduit AVANT d'essayer
// la lecture anglaise brute. Le R français est toujours le Roi (→ K).
const _FR_PIECES = { C: 'N', F: 'B', T: 'R', D: 'Q', R: 'K' };
function frSanToEnglish(san) {
  const s = (san || '').trim();
  if (!s || /^[Oo0]/.test(s)) return s;                       // roques inchangés
  let out = s.replace(/^([CFTDR])/, (_, p) => _FR_PIECES[p]); // pièce de tête
  out = out.replace(/=([CFTD])/, (_, p) => '=' + _FR_PIECES[p]); // promotion (jamais =R)
  return out;
}
...
// ── Numero de coup a la francaise : « 12. » (Blancs) / « 12… » (Noirs) ──────
// Le meme libelle est rendu par le repertoire, la liste des parties, les moments
// cles de la lecture et la page profil. Un seul helper : ecrit a la main, la
// forme derivait d'une surface a l'autre (9 copies avant le 04/08).
function moveLabel(moveNo, color) {
  return String(moveNo) + (color === 'b' ? '…' : '.');
}
...
export { _normFen, leitnerSchedule, DEFAULT_LADDER_HOURS, normalizeSAN, extractAllLines, pgnHeader,
         pgnStartFen, splitPgnGames, replacePgnGame, insertPgnGames, removePgnGame, pgnMainlineSans,
         frSanToEnglish,
         fig, PIECE_SYMS, figurineTitle, moveLabel,
         drillSelectGroups };
