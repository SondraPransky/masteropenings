...
function _setStudyLayout(on) {
  ...
  // ⚠ PAS display:none : la carte porte le <h1> de la page. Masquée en bloc, l'écran
  // le plus utilisé de l'app (l'étude d'un module) n'avait plus AUCUN titre — ni à
  // l'œil, ni pour un lecteur d'écran. `.study-hidden` masque tout SAUF le titre,
  // qui devient sr-only : la place gagnée par le chrome sur une ligne est conservée.
  if (info) info.classList.toggle('study-hidden', !!on);
  if (guessRow) guessRow.style.display = on ? 'flex' : 'none';
  ...
  _studyAllOpen = false;   // « Tout déplier » remis à zéro à l'entrée/sortie de l'étude
  const xb = document.getElementById('study-expand-btn');
  if (xb) { xb.classList.remove('active'); xb.setAttribute('aria-pressed', 'false'); }
  if (on) {
...
let _studyAllOpen  = false;       // bascule « Tout déplier » (comparer les lignes — leçon du fil Lichess « omg you changed it again »)
function studyToggleVar(key) {
  if (_studyOpenVars.has(key)) _studyOpenVars.delete(key); else _studyOpenVars.add(key);
  renderStudyTree();
}
function toggleStudyExpand() {
  _studyAllOpen = !_studyAllOpen;
  const b = document.getElementById('study-expand-btn');
  if (b) { b.classList.toggle('active', _studyAllOpen); b.setAttribute('aria-pressed', String(_studyAllOpen)); }
  renderStudyTree();
}
...
function renderStudyTree() {
  ...
  function branchHTML(v, vPath, depth) {
    ...
    const open = active || _studyAllOpen || _studyOpenVars.has(key);
    // .off (opacité .66) = branche RÉSUMÉE seulement : une branche dépliée (à la main
    // ou par « Tout déplier ») se lit à pleine encre — sinon tout l'arbre déplié
    // rendait estompé (audit 03/08).
    let h = `<div class="study-var ${active ? 'on' : (open ? '' : 'off')}">`;
    if (open) {
      h += ply(v, vPath, depth, true, true, true) + mainline(v, vPath, depth, false);
      if (!active && !_studyAllOpen) h += ` <span class="study-more" onclick="studyToggleVar('${key}')">replier</span>`;
    } else {
  ...
  // ── En-tête : sommaire CONTEXTUEL (retours utilisatrice 30/07, affiné 2×) — les
  // alternatives DU COUP COURANT uniquement (les autres enfants de sa position mère,
  // ex. sur 5…♘xe4 : ♘xb5 · c6 · ♗c5). PAS la dernière bifurcation traversée : au
  // 8.♘xh8, les options du 7e ne sont plus « ici » → pas de pastilles, fil d'Ariane.
  const numOf = (n) => `${n.fenBefore.split(' ')[5]}${n.fenBefore.split(' ')[1] === 'w' ? '.' : '…'}`;
  let forkPos = null, forkPath = [], forkTaken = -1, anyVar = false;
  if (curPath.length) {
    let pos = S.studyTree, ok = true;
    for (let i = 0; i < curPath.length - 1; i++) { pos = pos.children[curPath[i]]; if (!pos) { ok = false; break; } }
    const last = curPath[curPath.length - 1];
    if (ok && pos.children[last] && pos.children.length > 1) { forkPos = pos; forkPath = curPath.slice(0, -1); forkTaken = last; }
  }
  (function scan(n) { if (anyVar) return; if (n.children.length > 1) { anyVar = true; return; } n.children.forEach(scan); })(S.studyTree);
  let head = '';
  if (forkPos) {
    head = `<div class="study-toc"><span class="study-toc-lbl">Ici</span>` + forkPos.children.map((c, i) => {
      const p = [...forkPath, i];
      const on = i === forkTaken;
      const sr = branchSr(c);
      return `<span class="study-toc-t${on ? ' on' : ''}" onclick="studyGoPath(${JSON.stringify(p)})">${numOf(c)}${fig(c.san)}${nagGlyphs(c)}${sr.due > 0 ? '<span class="study-toc-d" title="À revoir"></span>' : ''}</span>`;
    }).join('') + `</div>`;
  } else if (anyVar) {
    // Pas encore d'embranchement sur le chemin : fil d'Ariane simple
    const segs = [];
...
Object.assign(window, {
  ...
  updateStudyProgress, _studyMastery, renderStudyTree, studyToggleVar, toggleStudyExpand,
