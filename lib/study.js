...
function renderStudyTree() {
  ...
  const masteredMark = '<span title="Maîtrisé" style="color:var(--green-ink);font-size:.82em;font-weight:700;margin-left:3px">✓</span>';
  const dueMark = '<span title="À revoir" style="display:inline-block;width:5px;height:5px;border-radius:50%;background:var(--gold);vertical-align:middle;margin-left:3px"></span>';
  ...
  const moveSpan = (node, path, depth, lead) => {
    ...
    return `<span ${isCur?'id="study-active" class="study-cur"':''} onclick="studyGoPath(${JSON.stringify(path)})" style="cursor:pointer;${lead||''}${s};${fs}padding:1px 3px;border-radius:var(--rs)">${fig(node.san)}${nagGlyphs(node)}</span>`;
  };
