...
import {
  CS, _tierPct, _masteryBadge, _studentAvatar,
...
function renderProfView(){
  ...
  document.getElementById('student-list').innerHTML = students.map(s => {
    ...
    // Plus de flag texte : « N à revoir » vit dans la pastille _masteryBadge, et
    // l'inactivité est désormais dite en clair par « vu il y a N j » (la teinte
    // ambre de la carte porte le signal « à suivre »). Dédup 03/08.
    const flag = '';
    return `<div class="eleve-item${isOn}${alertCls}" data-sname="${escapeHtml(s.key)}" data-search="${escapeHtml(_noAcc(s.label||''))}" role="button" tabindex="0" aria-pressed="${isOn?'true':'false'}" onclick="showStudentDetail(this.dataset.sname)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();showStudentDetail(this.dataset.sname)}">
      <div class="eleve-name">
        <span class="eleve-name-id">${_studentAvatar(s.key, s.label, 26)} <span class="eleve-name-txt">${escapeHtml(s.label)}</span></span>
        ${s.played ? _masteryBadge(s.label) : ''}
      </div>
      <!-- Stats EXPLICITES (03/08 : « 38j · 64% · 165 coups » était illisible — trois
           nombres sans étiquette) ; le point coloré de récence est retiré (info par la
           couleur seule, redondante avec « vu il y a … » et « inactif »). -->
      <div class="eleve-meta">${s.played ? (since===0?'vu aujourd\'hui':'vu il y a '+since+' j')+' · '+pct+' % de réussite · '+s.total+' coup'+(s.total>1?'s':'') : 'Pas encore commencé'}${flag}</div>
