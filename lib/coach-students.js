...
function renderProfView(){
  ...
  document.getElementById('student-list').innerHTML = students.map(s => {
    ...
    // Signal « à suivre » : l'inactivité seulement — le retard (« N à revoir ») est
    // DÉJÀ dit par la pastille _masteryBadge en haut de carte ; le redire ici
    // doublait l'info sur chaque carte en retard (passe better-interface 03/08).
    const flag = s.stale ? `<span class="eleve-flag stale">inactif ${since}j</span>` : '';
