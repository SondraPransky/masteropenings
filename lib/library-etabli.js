...
import { _normFen, fig, pgnMainlineSans, pgnStartFen, frSanToEnglish } from './core.js';
...
function _qeRenderPanels() {
  ...
  // Relecture guidée depuis la FEUILLE (arbitrage 28/07, socle sans OCR) :
  // le repère « où j'en suis sur ma feuille » suit chaque coup — c'est lui qui
  // évite de perdre sa ligne en recopiant.
  const turnEl = document.getElementById('lib-qe-turn');
  if (turnEl) {
    const f = g.fen().split(' ');
    turnEl.textContent = `Coup ${f[5]} — ${f[1] === 'w' ? 'Blancs' : 'Noirs'}`;
  }
...
// ── Saisie clavier depuis la feuille (notation française OU anglaise) ───────
// « Cf3 » se lit en français d'abord (C→N, F→B, T→R, D→Q, R→K), puis en
// anglais brut si la lecture française ne donne rien de légal. Le R français
// est TOUJOURS le Roi — c'est la déclaration de langue qui lève l'ambiguïté
// du corpus (ici la saisie vient d'une feuille française).
function qeTypeSubmit(ev) {
  ev?.preventDefault?.();
  const inp = /** @type {HTMLInputElement|null} */ (document.getElementById('lib-qe-san'));
  const raw = (inp?.value || '').trim();
  if (!raw) return false;
  const g = _game();
  const mv = g.move(frSanToEnglish(raw), { sloppy: true }) || g.move(raw, { sloppy: true });
  if (!mv) {
    toast(`⚠ « ${raw} » n'est pas jouable ici — vérifie ta feuille (coup ${g.fen().split(' ')[5]})`, 'ko');
    inp?.select?.();
    return false;
  }
  if (inp) inp.value = '';
  _qeSync();
  inp?.focus?.();                    // la frappe enchaîne, coup après coup
  return false;
}
...
Object.assign(window, { _qeMount, qeUndo, qeReset, qePlaySan, qeSave, qeTypeSubmit });

export { _qeMount, qeUndo, qeReset, qePlaySan, qeSave, qeTypeSubmit };
