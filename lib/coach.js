...
function renderProfView(){
  ...
  const _behindN = students.filter(s => s.behind).length;
  ...
  const attColor = _behindN > 0 ? 'var(--red)' : needAttention > 0 ? 'var(--gold)' : 'var(--green)';
  document.getElementById('prof-kpis').innerHTML=`
    ...
    <div class="cs-kpi${_behindN>0?' cs-kpi-accent':''}"><div class="cs-kpi-val" style="color:${attColor}">${students.length?needAttention:'—'}</div><div class="cs-kpi-lbl">À suivre${_behindN>0?` · ${_behindN} en retard`:''}</div></div>
...
function showStudentDetail(id) {
  ...
  if (topErrors.length) {
    resumeHtml += `<div class="ed-review-sec">
      <div class="ed-review-title"><i class="ti ti-target" aria-hidden="true"></i> À revoir avec cet élève</div>`;
    topErrors.forEach(e => {
