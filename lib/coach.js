    ...
    topErrors.forEach(e => {
      ...
      resumeHtml += `<div class="ed-review-item" tabindex="0" data-did="${escapeHtml(String(e.drillId))}" data-san="${escapeHtml(e.san||'')}"
        ...
        <button class="btn btn-blue btn-sm btn-ico" title="Assigner cette révision à l'élève" aria-label="Assigner cette révision à l'élève"
          onclick="event.stopPropagation();var it=this.closest('.ed-review-item');assignReviewForStudent(it.dataset.did,it.dataset.san,this)"><i class="ti ti-target" aria-hidden="true"></i></button>
      </div>`;
    });
    resumeHtml += '</div>';
  }
...
// Assigner une révision ciblée depuis le DÉTAIL d'un élève (≠ Points faibles qui indexe
// _wsCards). Route la position vers les classes où CET élève est inscrit ET qui ont le
// module. Matching tolérant email/pseudo/nom (les rosters stockent des emails, la clé
// sélectionnée peut être un nom d'affichage).
function assignReviewForStudent(drillId, san, btn) {
  if (!selectedStudent) return;
  const kl = String(selectedStudent).toLowerCase();
  // identifiants complets de l'élève (email canonique inclus) depuis ses résultats
  const ids = new Set([kl]);
  G.results.forEach(r => {
    const rk = [(r.studentEmail||'').toLowerCase(),(r.studentPseudo||'').toLowerCase(),(r.student||'').toLowerCase()];
    if (rk.includes(kl)) rk.forEach(x => x && ids.add(x));
  });
  ...
  G.classes.forEach(cls => {
    ...
    if (!Array.isArray(cls.targetedReviews)) cls.targetedReviews = [];
    let rev = cls.targetedReviews.find(r => String(r.drillId)===String(drillId) && r.san===san);
    if (rev) {
      if (!(rev.students||[]).map(s=>String(s).toLowerCase()).includes(email.toLowerCase())) rev.students = [...(rev.students||[]), email];
      rev.assignedAt = Date.now();
    } else {
      cls.targetedReviews.push({ id: Date.now()+'_'+Math.random().toString(36).slice(2,6), drillId, drillName: drill?drill.name:'', san, fen: null, comment: hit?hit.comment:'', students: [email], assignedAt: Date.now() });
    }
    window._sbSaveClass?.(cls);
    reached++;
  });
  if (!reached) { toast('Cet élève n\'est dans aucune classe ayant ce module', 'ko'); return; }
  window.saveClasses?.();
  toast(`✓ « ${san} » à revoir assigné à l'élève`, 'ok');
  if (btn) { btn.innerHTML = '<i class="ti ti-check" aria-hidden="true"></i>'; btn.setAttribute('disabled','disabled'); }
}
