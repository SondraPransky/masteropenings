...
function _sharedGamesHTML() {
  ...
  const rows = shared.map(g => {
    ...
    const nat = g.nature==='analyse' ? '📝 Analyse' : '♟️ Partie';
    const badge = g.reviewedAt ? `<span title="Déjà annotée" style="color:#7c3aed;font-weight:700;font-size:.7rem">✨ Annotée</span>` : '';
    return `<div class="game-row">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        <div style="display:flex;align-items:center;gap:10px">
          <span class="game-result ${g.result==='1-0'?'win':g.result==='0-1'?'loss':'draw'}">${g.result||'*'}</span>
          <div>
            <div style="font-weight:600;font-size:.85rem">👤 ${escapeHtml(g.student||'Élève')} — ${escapeHtml(who)} ${badge}</div>
            <div style="font-size:.72rem;color:var(--dim)">${nat} · ${escapeHtml(g.event||'—')} · ${dt}</div>
          </div>
        </div>
        <button class="btn btn-gold btn-sm" onclick="annotateSharedGame('${g.id}')">🧑‍🏫 ${g.reviewedAt?'Revoir la revue':'Annoter'}</button>
      </div>
    </div>`;
  }).join('');
