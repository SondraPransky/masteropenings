...
function showStudentDetail(id) {
  ...
  if (topErrors.length) {
    resumeHtml += `<div style="border-top:1px solid var(--border);margin-top:12px;padding-top:12px">
      <div style="font-size:.72rem;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px"><i class="ti ti-target" aria-hidden="true"></i> À revoir avec cet élève</div>`;
    topErrors.forEach(e => {
      const rate = Math.round(e.fails / e.attempts * 100);
      const rc = rate>=60?'var(--red)':rate>=30?'#ca8a04':'var(--green)';
...
function openWeakspotPosition(i) {
  ...
  body.innerHTML = `<div style="display:flex;gap:18px;flex-wrap:wrap;align-items:flex-start">
    ...
    <div class="wsx-actions" style="margin-top:18px">
      <button class="btn btn-primary btn-sm" data-assign="${i}" onclick="assignTargetedReview(${i})"><i class="ti ti-target" aria-hidden="true"></i> Assigner cette révision aux élèves</button>
    </div>`;
...
// dans les révisions ciblées des classes contenant les élèves qui échouent → ils la voient
// sur leur accueil. Persisté migration-free dans class.targetedReviews (→ classes.extra).
function assignTargetedReview(i) {
  const p = _wsCards[i]; if (!p) return;
  const failSet = new Set(p.failStudents.map(s => String(s).toLowerCase()));
  const reached = new Set();
  const touched = [];
  G.classes.forEach(cls => {
    const targets = (cls.studentEmails || cls.students || []).map(String).filter(s => failSet.has(s.toLowerCase()));
    ...
    if (rev) { rev.students = [...new Set([...(rev.students || []), ...targets])]; rev.assignedAt = Date.now(); }
    else cls.targetedReviews.push({ id: Date.now() + '_' + Math.random().toString(36).slice(2, 6), drillId: p.drillId, drillName: p.drillName, san: p.san, fen: p.fen || null, comment: p.comment || '', students: targets, assignedAt: Date.now() });
    targets.forEach(t => reached.add(t.toLowerCase()));
    touched.push(cls);
  });
  if (!reached.size) { toast('Aucun de ces élèves n\'est dans une classe', 'ko'); return; }
  window.saveClasses?.();
  touched.forEach(cls => window._sbSaveClass?.(cls));
...
Object.assign(window, {
  ...
  annotateSharedGame, openWeakspotPosition, assignTargetedReview,
});
