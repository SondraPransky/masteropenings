...
const _tierPct  = (pct)  => pct  >= 70 ? 'var(--green)'     : pct  >= 50 ? 'var(--gold)'     : 'var(--red)';      // réussite : haut = bon
const _tierBg   = (pct)  => pct  >= 70 ? 'var(--green-dim)' : pct  >= 50 ? 'var(--gold-dim)' : 'var(--red-dim)';  // fond dim assorti
const _tierFail = (rate) => rate >= 60 ? 'var(--red)'       : rate >= 30 ? 'var(--gold)'     : 'var(--green)';    // taux d'échec : haut = mauvais
...
function renderClassWeakSpots(arr) {
  ...
  el.innerHTML = `<div class="wsx-panel">
    ...
    <div class="wsx-panel-sub">Aperçu des positions qui font le plus échouer tes élèves — à revoir en cours.</div>
    ${spots.map(s => `<div class="wsx-row">
      <span class="wsx-move">${fig(s.san)}</span>
      <div class="wsx-row-info">
        <div class="wsx-row-name">${escapeHtml(s.drillName || 'Module')}${s.comment ? ` · <span class="wsx-comment-inline">${escapeHtml(s.comment.slice(0,42))}</span>` : ''}</div>
      </div>
      <span class="wsx-rate" style="background:${_tierBg(100-s.rate)};color:${_tierFail(s.rate)};white-space:nowrap">${s.failStudentCount} élève${s.failStudentCount > 1 ? 's' : ''} · ${s.rate}% d'échec</span>
    </div>`).join('')}
  </div>`;
}
...
function _renderCoachLoading() {
  ...
  const kpis = document.getElementById('prof-kpis');
  ...
  const ws = document.getElementById('prof-weakspots'); if (ws) ws.innerHTML = '';
...
function _renderCoachError() {
  ...
  const ui = document.getElementById('prof-ui'); if (ui) ui.style.display = '';
  const kpis = document.getElementById('prof-kpis');
  if (kpis) kpis.innerHTML = `<div class="coach-error">
  ...
  ['prof-weakspots','student-list','prof-detail'].forEach(id => { const e = document.getElementById(id); if (e) e.innerHTML = ''; });
}
...
function renderProfView(){
  ...
  // Activité + « à suivre » (élèves qui demandent l'attention du coach) : positions
  // dues en révision espacée OU inactifs depuis >7j. C'est le KPI actionnable.
  const _now = Date.now();
  ...
  const _dueBy = s => Object.keys(G.masteryData).some(k => k.startsWith(s.label+'_') && G.masteryData[k].due<=_now);
  const needAttention = students.filter(s => _dueBy(s) || (s.played && s.lastTs < _wkAgo)).length;
  const maiaN = G.savedGames.filter(g=>!g.baseId).length;

  // KPI strip PRESCRIPTIF : 3 tuiles qui portent une décision (dont « À suivre »
  // en accent), pas 4 chiffres égaux. Vanité (sessions/parties) démotée en note.
  const attColor = needAttention > 0 ? 'var(--red)' : 'var(--green)';
  document.getElementById('prof-kpis').innerHTML=`
    ...
    <div class="cs-kpi${needAttention>0?' cs-kpi-accent':''}"><div class="cs-kpi-val" style="color:${attColor}">${students.length?needAttention:'—'}</div><div class="cs-kpi-lbl">À suivre</div></div>
    <div class="cs-kpi-note">${students.length?`<i class="ti ti-refresh" aria-hidden="true"></i> ${_activeWk} actif${_activeWk>1?'s':''} cette semaine · ${_inactive} inactif${_inactive>1?'s':''} (&gt;7j)`:''}<span class="cs-kpi-note-sep">${sessions.length} session${sessions.length>1?'s':''} · ${maiaN} partie${maiaN>1?'s':''} Maia</span></div>`;
  ...
  document.getElementById('student-list').innerHTML = students.map(s => {
    ...
    const alertCls = _dueBy(s) ? ' alert' : '';
    const dotColor = !s.played ? 'var(--dim)' : since===0 ? 'var(--green)' : since<=7 ? 'var(--gold)' : 'var(--dim)';
    const isOn = s.key===selectedStudent ? ' on' : '';
    return `<div class="eleve-item${isOn}${alertCls}" data-sname="${escapeHtml(s.key)}" role="button" tabindex="0" aria-pressed="${isOn?'true':'false'}" onclick="showStudentDetail(this.dataset.sname)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();showStudentDetail(this.dataset.sname)}">
...
function _eleveListKey(e) {
  ...
  const items = [...document.querySelectorAll('#student-list .eleve-item')];
...
function showStudentDetail(id) {
  ...
  sr.forEach(r => {
    ...
    if (!errMap[key]) errMap[key] = { drillName: r.drillName, san: r.san, comment: r.comment || '', fails: 0, attempts: 0 };
    errMap[key].attempts++;
  ...
  if (topErrors.length) {
    resumeHtml += `<div style="border-top:1px solid var(--border);margin-top:12px;padding-top:12px">
      <div style="font-size:.8rem;font-weight:700;color:var(--text-2);margin-bottom:8px"><i class="ti ti-target" aria-hidden="true"></i> À revoir avec cet élève</div>`;
    topErrors.forEach(e => {
      const rate = Math.round(e.fails / e.attempts * 100);
      const rc = _tierFail(rate);
      resumeHtml += `<div class="ed-review-item">
        <span class="ed-review-move">${fig(e.san||'?')}</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:.74rem;color:var(--dim)">${escapeHtml(e.drillName)} · raté ${e.fails}×</div>
          ${e.comment?`<div style="font-size:.75rem;color:var(--text-2);font-style:italic;line-height:1.4">« ${figText(escapeHtml(e.comment.slice(0,90)))}${e.comment.length>90?'…':''} »</div>`:''}
        </div>
        <span style="color:${rc};font-weight:700;font-size:.82rem;flex-shrink:0">${rate}%</span>
      </div>`;
    });
    resumeHtml += '</div>';
  }
    ...
    posHtml += `<div style="margin-bottom:22px">
      ...
      <table class="pos-table">
        ...
        <tbody>${posArr.map(p => {
          ...
          const due = p.sm2 ? (p.sm2.due<=now ? '<span class="mastery-pill low">À revoir</span>' : `<span class="mastery-pill ok">dans ${Math.ceil((p.sm2.due-now)/86400000)}j</span>`) : '<span style="color:var(--dim)">—</span>';
          return `<tr>
...
function _buildProgressionHTML(name) {
  ...
  if (log.length) {
    ...
    html += `<div class="ed-subhead" style="margin-bottom:12px"><i class="ti ti-trending-up" aria-hidden="true"></i> Progression</div>`;
  ...
  if (topErrors.length) {
    const now = Date.now();
    html += `<div class="ed-subhead" style="margin:${log.length?'16px 0':'0 0'} 10px"><i class="ti ti-trending-down" aria-hidden="true"></i> Positions difficiles</div>`;
    html += `<table class="pos-table" style="font-size:.78rem"><thead><tr><th>Module</th><th>Coup</th><th>Échecs</th><th>Taux</th></tr></thead><tbody>`;
    topErrors.forEach(e=>{
      const rate = Math.round(e.fails/e.attempts*100);
      const rateCol = _tierFail(rate);
      html += `<tr>
        ...
        <td><span class="mono-move">${escapeHtml(e.san||'?')}</span></td>
...
function renderHeatmap() {
  ...
  const rateCol = _tierPct;
  const rateBg  = _tierBg;
  ...
  const cards = entries.filter(p=>p.failStudents.size>0)
    ...
    .slice(0, 8);
  _wsCards = cards.map(p => ({ drillId:p.drillId, drillName:p.drillName, san:p.san, comment:p.comment, rate:p.rate, students:[...p.students], failStudents:[...p.failStudents], fen:_drillFenMap(p.drillId)[p.san] || null }));
  const cardsHtml = _wsCards.map((p,i) => {
    const col = rateCol(p.rate), fs = p.failStudents.length;
    const chips = p.failStudents.slice(0,10).map(s=>`<span class="wsx-chip">${escapeHtml(s)}</span>`).join('');
    const board = p.fen ? renderStaticBoard(p.fen, { size: 76 }) : '';
    return `<div class="wsx-card${p.rate<50?' hot':''}">
      <div class="wsx-card-main">
        ${board}
        <div style="min-width:0;flex:1">
          ...
          <div class="wsx-sub">${fs} / ${p.students.length} élève${p.students.length>1?'s':''} échoue${fs>1?'nt':''}</div>
          <div class="wsx-chips">${chips}</div>
        </div>
      </div>
      ${p.comment?`<div class="wsx-comment">« ${figText(escapeHtml(p.comment.slice(0,140)))}${p.comment.length>140?'…':''} »</div>`:''}
      <div class="wsx-actions">
        <button class="btn btn-blue btn-sm" onclick="openWeakspotPosition(${i})"><i class="ti ti-eye" aria-hidden="true"></i> Voir</button>
        <button class="btn btn-primary btn-sm" data-assign="${i}" onclick="assignTargetedReview(${i})"><i class="ti ti-target" aria-hidden="true"></i> Assigner une révision</button>
  ...
  const stripHtml = strip.map(p=>{
    ...
    const who = [...p.students].join(', ');
    // Focusable + aria-label : les élèves ne sont plus cachés derrière le seul hover (clavier/SR/tactile).
    return `<div class="wsx-cell" style="background:${rateBg(p.rate)};border-color:${col}" tabindex="0" title="${escapeHtml(who)}" aria-label="${escapeHtml(fig(p.san))} — ${p.rate}% de réussite. Élèves : ${escapeHtml(who)}">
...
function _sharedGamesHTML() {
  ...
  return `<div style="margin-bottom:18px">
    <div style="font-weight:700;font-size:.9rem;margin-bottom:8px"><i class="ti ti-inbox" aria-hidden="true"></i> Parties partagées par les élèves <span style="color:var(--dim);font-weight:400">(${shared.length})</span></div>
    ${rows}
  </div>`;
}
...
// Dernière assignation (pour l'undo) : { i, patches:[{clsId, revId, added:[], created}] }.
let _lastAssign = null;

// « Assigner une révision ciblée » : pousse la position (module + coup + FEN + commentaire)
// dans class.targetedReviews (→ classes.extra, migration-free). Deux portées :
//  - défaut : uniquement les élèves qui échouent (précision) ;
//  - {whole:true} : toute la classe à qui le module est assigné (prévention).
// Capture de quoi ANNULER (undoTargetedReview) : l'action n'est plus irréversible.
function assignTargetedReview(i, opts = {}) {
  const p = _wsCards[i]; if (!p) return;
  const whole = !!opts.whole;
  const failSet = new Set(p.failStudents.map(s => String(s).toLowerCase()));
  const reached = new Set();
  const patches = [];
  G.classes.forEach(cls => {
    const roster = (cls.studentEmails || cls.students || []).map(String);
    const targets = whole
    ...
    let rev = cls.targetedReviews.find(r => String(r.drillId) === String(p.drillId) && r.san === p.san);
    if (rev) {
      ...
      cls.targetedReviews.push({ id: revId, drillId: p.drillId, drillName: p.drillName, san: p.san, fen: p.fen || null, comment: p.comment || '', students: targets, assignedAt: Date.now() });
      patches.push({ clsId: cls.id, revId, added: targets, created: true });
    }
    targets.forEach(t => reached.add(t.toLowerCase()));
  });
  if (!reached.size) { toast(whole ? 'Aucune classe n\'a ce module assigné' : 'Aucun de ces élèves n\'est dans une classe', 'ko'); return; }
  window.saveClasses?.();
  patches.forEach(u => { const c = G.classes.find(x => x.id === u.clsId); if (c) window._sbSaveClass?.(c); });
  _lastAssign = { i, patches };
  toast(`✓ « ${p.san} » à réviser assigné à ${reached.size} élève${reached.size > 1 ? 's' : ''}`, 'ok');
...
function undoTargetedReview(i) {
  if (!_lastAssign || _lastAssign.i !== i) return;
  ...
  document.querySelectorAll(`[data-assign="${i}"]`).forEach(b => { b.innerHTML = '<i class="ti ti-target" aria-hidden="true"></i> Assigner'; b.removeAttribute('disabled'); });
  document.querySelectorAll(`[data-undo="${i}"]`).forEach(b => { b.style.display = 'none'; });
}
...
Object.assign(window, {
  ...
  annotateSharedGame, openWeakspotPosition, assignTargetedReview, undoTargetedReview,
  _eleveListKey,
});
