...
function renderOverview() {
  ...
  const attColor   = behindN > 0 ? 'var(--red)' : needAttention > 0 ? 'var(--gold)' : 'var(--green)';
  ...
  if (inEl) {
    ...
    inEl.innerHTML = `<div class="wsx-panel">
      ...
      ${todo.length ? todo.map(g => `<button class="ov-row" onclick="annotateSharedGame('${escapeHtml(String(g.id))}')">
        <span class="ov-row-name"><i class="ti ti-chess" aria-hidden="true"></i> ${escapeHtml(g.student || 'Élève')} · ${escapeHtml((g.white||'?') + ' – ' + (g.black||'?'))}</span>
        <span class="ov-row-meta">${g.result || '*'}</span>
      </button>`).join('') : `<div class="ov-ok"><i class="ti ti-circle-check" aria-hidden="true"></i> Rien en attente.</div>`}
    </div>`;
  }
}
...
function showStudentDetail(id) {
  ...
  G.classes
    .filter(c => (c.studentEmails || c.students || []).some(e => String(e).toLowerCase() === idLower))
    .forEach(c => {
...
function renderHeatmap() {
  ...
  filtered.forEach(r => {
    ...
    if (r.correct) p.correct++;
      ...
      // Identifiants de matching (email/pseudo/nom) : les rosters de classes stockent des
      // emails alors que r.student est un nom d'affichage → sans ça, l'assignation ciblée
      // ne retrouve jamais les élèves.
      [(r.studentEmail||''), (r.studentPseudo||''), (r.student||'')].forEach(k => { if (k) p.failIds.add(String(k).toLowerCase()); });
    }
  ...
  const sections = [sel].map(m => {
    const rowsHtml = m.rows.map(p => {
      ...
      return `<tr class="wsx-tr${p.rate<50?' hot':''}" tabindex="0" data-did="${escapeHtml(String(p.drillId))}" data-san="${escapeHtml(p.san)}"
        ...
        <td class="wsx-act">
          <button class="btn btn-ghost btn-sm" onclick="openWeakspotPosition(${i})" title="Voir la position"><i class="ti ti-eye" aria-hidden="true"></i></button>
          <button class="btn btn-blue btn-sm" data-assign="${i}" onclick="assignTargetedReview(${i})" title="Assigner une révision ciblée"><i class="ti ti-target" aria-hidden="true"></i></button>
          <button class="btn btn-ghost btn-sm" data-undo="${i}" onclick="undoTargetedReview(${i})" style="display:none" title="Annuler l'assignation"><i class="ti ti-arrow-back-up" aria-hidden="true"></i></button>
        </td>
      </tr>`;
    }).join('');
...
function _classDetailHTML(cls) {
  ...
  return `<div class="cls-detail-head">
    ...
    <div class="cls-detail-title">
      <div style="font-size:1.05rem;font-weight:700"><i class="ti ${cls.individual?'ti-user':'ti-school'}" aria-hidden="true"></i> ${escapeHtml(name)}</div>
    ...
    <div style="display:flex;gap:6px;flex-shrink:0">
      ...
      <button class="btn btn-ghost btn-sm" onclick="deleteClass(${cls.id})" title="Supprimer"><i class="ti ti-trash" aria-hidden="true"></i></button>
    </div>
  </div>
...
function _classBreakdownHTML(cls) {
  ...
  return modIds.map(modId => {
    ...
    const stuRows = roster.map(email => {
      ...
      return `<button class="ov-row cls-stu-row" onclick="ovOpenStudent(this.dataset.k)" data-k="${escapeHtml(email)}">
        <span class="ov-row-name"><i class="ti ti-point-filled" style="color:${played?'var(--green)':(late?'var(--red)':'var(--dim)')}" aria-hidden="true"></i> ${escapeHtml(email)}</span>
