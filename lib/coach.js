...
function renderClassesTab() {
  ...
  if (!el) return;
  if (!G.classes.length) {
    el.innerHTML = '<div class="empty" style="padding:40px;border:1px dashed var(--border);border-radius:var(--r)"><div class="empty-ico"><i class="ti ti-school" aria-hidden="true"></i></div>Aucune classe.<br>Créez-en une à gauche pour suivre vos élèves.</div>';
    return;
  }
  const now = Date.now();
  ...
  const rows = shared.map(g => {
    ...
    return `<div class="game-row">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        ...
        <button class="btn btn-gold btn-sm" onclick="annotateSharedGame('${g.id}')"><i class="ti ti-school" aria-hidden="true"></i> ${g.reviewedAt?'Revoir la revue':'Annoter'}</button>
      </div>
    </div>`;
  }).join('');
