...
function _expCell(sq, piece, r, f) {
  ...
  // Pièce du camp au trait = déplaçable (drag & drop, MÊME patron que l'éditeur :
  // HTML5 drag + ghost partagé #ghost-canvas ; le tactile vit sur la case).
  ...
  return `<div class="exp-sq${dark ? ' exp-sq-d' : ''}${sel}${hint}" data-sq="${sq}"${drag}
...
// Exécuteur commun d'un coup (clic, drag souris, drag tactile) : gère la
// promotion via le sélecteur PARTAGÉ (showPromoPicker, comme drill et éditeur)
// et route vers le solveur ou l'exploration.
function _expTryMove(from, to, cx, cy) {
  ...
  // coup illégal : re-sélectionne la destination si c'est une pièce au trait
  const pc = g.get(to);
...
// ── Drag & drop souris (patron editorDragStart/Drop, ghost = la pièce seule) ──
let _expDragSq = null;
...
document.addEventListener('touchend', e => {
  ...
  if (Math.hypot(dx, dy) < 8) {           // tap = clic (sélection / coup en 2 taps)
