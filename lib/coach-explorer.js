...
// si le pont ne répond pas. L'échiquier est un composant clic/drag AUTONOME
// (chess.js + re-rendu) — il ne réutilise PAS lib/board.js (zone critique,
// couplée au drill) ; son drag & drop reprend le patron de l'éditeur.
// ══════════════════════════════════════════════════════
...
// ── Échiquier (drag & drop patron éditeur + confort Chessground) ──────
function _expRenderBoard() {
  ...
  // Confort (l'équivalent des signaux Chessground de l'original) : dernier coup
  // surligné, destinations légales de la pièce sélectionnée, roi en échec.
  const hist = g.history({ verbose: true });
  const last = hist.length ? hist[hist.length - 1] : null;
  const dests = new Map();   // sq → 'dest' | 'capt'
  if (_expSelSq) {
    for (const m of g.moves({ square: _expSelSq, verbose: true })) {
      dests.set(m.to, m.captured ? 'capt' : 'dest');
    }
  }
  let checkSq = null;
  if (g.in_check()) {
    ...
    for (let r = 0; r < 8; r++) for (let f = 0; f < 8; f++) {
      ...
      const p = g.get(sq);
      if (p && p.type === 'k' && p.color === turn) { checkSq = sq; break outer; }
    }
  }
  let cells = '';
  for (let dr = 0; dr < 8; dr++) {
    for (let df = 0; df < 8; df++) {
      ...
      const sq = files[f] + (8 - r);
      // coordonnées dans les cases du bord AFFICHÉ (style Chessground, comme
      // l'outil original) : lettres sur la rangée du bas, chiffres sur la colonne
      // de gauche — repères indispensables pour qui tape ensuite des coups UCI.
      let coord = '';
      if (dr === 7) coord += `<span class="exp-coord exp-coord-f">${files[f]}</span>`;
      if (df === 0) coord += `<span class="exp-coord exp-coord-r">${8 - r}</span>`;
      cells += _expCell(sq, grid[r][f], r, f, { last, dests, checkSq, coord });
    }
  }
...
function _expCell(sq, piece, r, f, sig) {
  const g = _expGame;
  const dark = (r + f) % 2 === 1;
  let cls = 'exp-sq' + (dark ? ' exp-sq-d' : '');
  if (_expSelSq === sq) cls += ' exp-sq-sel';
  if (_expHintSq === sq) cls += ' exp-sq-hint';
  if (sig.last && (sig.last.from === sq || sig.last.to === sq)) cls += ' exp-sq-last';
  if (sig.checkSq === sq) cls += ' exp-sq-check';
  const d = sig.dests.get(sq);
  if (d === 'dest') cls += ' exp-sq-dest';
  if (d === 'capt') cls += ' exp-sq-capt';
  const canDrag = !!piece && piece[0] === g.turn() && !(_expPz && _expPz.solved);
  const drag = canDrag ? ` draggable="true" ondragstart="_expDragStart(event,'${sq}')"` : '';
  const pc = piece ? `<img src="${window.PIECE_CDN}${piece}.svg" draggable="false" alt="">` : '';
  return `<div class="${cls}" data-sq="${sq}"${drag}
    onclick="_expClickSq(event,'${sq}')" ontouchstart="_expTouchStart(event,'${sq}')"
    ondragover="event.preventDefault()" ondrop="_expDrop(event,'${sq}')" ondragend="_expDragEnd()"
    ${canDrag ? 'style="cursor:grab"' : ''}>${sig.coord || ''}${pc}</div>`;
}
...
// Exécuteur commun d'un coup (clic, drag souris, drag tactile) : promotion via
// le sélecteur PARTAGÉ (showPromoPicker), routage solveur/exploration.
function _expTryMove(from, to, cx, cy) {
  const g = _expGame;
  if (!from || !to || from === to) { _expSelSq = null; _expRenderBoard(); return; }
  if (_expPz && _expPz.solved) { _expSelSq = null; _expRenderBoard(); return; }
  const done = (mv) => {
    _expSelSq = null;
  ...
  if (isPromo) {
    window.showPromoPicker?.(mp.color, cx || 0, cy || 0, pr => {
      const mv = g.move({ from, to, promotion: pr });
      if (mv) done(mv); else { _expSelSq = null; _expRenderBoard(); }
    });
    return;
  }
  const mv = g.move({ from, to, promotion: 'q' });
  if (mv) { done(mv); return; }
  const pc = g.get(to);
  _expSelSq = (pc && pc.color === g.turn()) ? to : null;
  _expRenderBoard();
}

function _expClickSq(e, sq) {
  if (_expSelSq && _expSelSq !== sq) { _expTryMove(_expSelSq, sq, e?.clientX, e?.clientY); return; }
  const pc = _expGame.get(sq);
  _expSelSq = (pc && pc.color === _expGame.turn()) ? sq : null;
  _expRenderBoard();
}

// ── Drag & drop souris (patron editorDragStart/Drop, ghost partagé) ──
let _expDragSq = null;
function _expDragStart(e, sq) {
  const g = _expGame, p = g.get(sq);
  if (!p || p.color !== g.turn()) { e.preventDefault(); return; }
  _expDragSq = sq; _expSelSq = sq;
  e.dataTransfer.effectAllowed = 'move';
  e.dataTransfer.setData('text/plain', sq);
  const size = e.target.closest('[data-sq]')?.offsetWidth || 36;
  ...
  if (gc) {
    ...
    e.dataTransfer.setDragImage(gc, size / 2, size / 2);
    requestAnimationFrame(() => { gc.style.display = 'none'; });
  }
  // re-rendu APRÈS le dragstart (sinon le navigateur annule le drag)
  requestAnimationFrame(() => _expRenderBoard());
}
function _expDrop(e, sq) {
  e.preventDefault();
  const from = _expDragSq; _expDragSq = null;
  _expTryMove(from, sq, e.clientX, e.clientY);
}
function _expDragEnd() {
  if (_expDragSq) { _expDragSq = null; _expSelSq = null; _expRenderBoard(); }
}

// ── Drag tactile (patron editorTouchStart : ghost suiveur + elementFromPoint) ──
let _expTouchFrom = null, _expTouchXY = { x: 0, y: 0 }, _expTouchSize = 36;
function _expTouchStart(e, sq) {
  if (document.getElementById('promo-pick')?.style.display === 'flex') return;
  e.preventDefault();
  const t = e.touches[0]; if (!t) return;
  _expTouchFrom = sq; _expTouchXY = { x: t.clientX, y: t.clientY };
  const g = _expGame, p = g.get(sq);
  if (p && p.color === g.turn() && !(_expPz && _expPz.solved)) {
    const size = _expTouchSize = e.target.closest('[data-sq]')?.offsetWidth || 36;
    const gc = document.getElementById('ghost-canvas');
    if (!gc) return;
    gc.width = size; gc.height = size;
    const ctx = gc.getContext('2d');
    ctx.clearRect(0, 0, size, size);
    const img = window.pieceImgs?.[p.color + p.type.toUpperCase()];
    if (img?.complete) { ctx.globalAlpha = .85; ctx.drawImage(img, 0, 0, size, size); ctx.globalAlpha = 1; }
    gc.style.cssText = `position:fixed;left:${t.clientX - size / 2}px;top:${t.clientY - size / 2}px;width:${size}px;height:${size}px;pointer-events:none;display:block;z-index:var(--z-drag)`;
  }
}
document.addEventListener('touchmove', e => {
  if (!_expTouchFrom) return;
  e.preventDefault();
  const t = e.touches[0]; if (!t) return;
  const gc = document.getElementById('ghost-canvas');
  if (gc) { gc.style.left = (t.clientX - _expTouchSize / 2) + 'px'; gc.style.top = (t.clientY - _expTouchSize / 2) + 'px'; }
}, { passive: false });
document.addEventListener('touchend', e => {
  ...
  const from = _expTouchFrom; _expTouchFrom = null;
  if (!t) return;
  const dx = t.clientX - _expTouchXY.x, dy = t.clientY - _expTouchXY.y;
  if (Math.hypot(dx, dy) < 8) {
    _expClickSq({ clientX: t.clientX, clientY: t.clientY }, from);
    return;
  }
  const cell = document.elementFromPoint(t.clientX, t.clientY)?.closest('[data-sq]');
  _expTryMove(from, cell?.dataset.sq, t.clientX, t.clientY);
});
...
Object.assign(window, {
  ...
  _expToggleSel, _expToggleAll, _expClickSq, _expPrev, _expDetectBridge,
  _expDragStart, _expDrop, _expDragEnd, _expTouchStart,
});
