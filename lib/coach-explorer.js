...
function _expRenderBoard() {
  ...
  for (let dr = 0; dr < 8; dr++) {
    for (let df = 0; df < 8; df++) {
      ...
      const sq = files[f] + (8 - r);
      cells += _expCell(sq, grid[r][f], r, f, { last, dests, checkSq });
    }
  }
...
function _expCell(sq, piece, r, f, sig) {
  ...
  return `<div class="${cls}" data-sq="${sq}"${drag}
    ...
    ${canDrag ? 'style="cursor:grab"' : ''}>${pc}</div>`;
}
...
async function _expGo() {
  ...
  try {
    ...
    if (ctx.ply >= 2) {
      ...
      if (guard) guard.style.display = ctx.ply > 0 || _expUci.length ? '' : 'none';
...
// Export PGN direct (le dialogue calibré arrive en tranche suivante).
function explorerOpenExport() { explorerExportPgn(); }

async function explorerExportPgn() {
  if (!_expNfen) { window.toast?.('⚠ Explore d\'abord une position', 'ko'); return; }
  try {
    const r = await fetch(`${ODP_BRIDGE_URL}/export?${_expPosParam()}&limit=200&sort=${_expSort}${_expRatingParam()}`);
...
Object.assign(window, {
  ...
  explorerOpenExport, explorerResetBoard, explorerPickOpening,
