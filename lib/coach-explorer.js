...
import { ODP_BRIDGE_URL, EX, playUci, toggleLevel, levelChipsHTML } from './coach-explorer-core.js';
...
// Niveaux élève FIDE (EX.levels/EX.levelDefs) : partagés avec le dialogue
// d'export (mêmes pastilles que l'outil OTKB — la vérité vit dans levels.py,
// servie par /levels). « all » = exclusif. `EX.nfen` = FEN normalisée courante.
...
async function _expLoadLevels() {
  if (EX.levelDefs.length) { _expRenderLevels(); return; }
  try {
    const r = await fetch(ODP_BRIDGE_URL + '/levels');
    EX.levelDefs = (await r.json()).levels || [];
    _expRenderLevels();
...
function _expRenderLevels() {
  ...
  if (host) host.innerHTML = levelChipsHTML(EX.levelDefs, EX.levels, 'explorerToggleLevel');
}

function explorerToggleLevel(key) {
  EX.levels = toggleLevel(EX.levels, key, EX.levelDefs);
  _expOffset = 0;
  _expRenderLevels();
  _expLoadThrough();
}
...
function _expLoadCg() {
  if (!_cgLoad) _cgLoad = loadChessground();   // loader partagé (lib/chessground.js)
  return _cgLoad;
}
...
// ⚠️ Dans le build Chessground vendoré, `setAutoShapes` met à jour l'état mais ne
// REPEINT PAS le SVG des formes (même bug que lib/board.js, mesuré le 27/07) →
// sans ce redrawAll le cercle d'indice du solveur ne s'affichait jamais. Garde de
// signature : on ne repeint que quand la forme change, pas à chaque coup.
let _expLastShapeSig = '';
async function _expRenderBoard() {
  const cg = await _expEnsureCg();
  if (!cg) return;
  cg.set(_expBoardCfg());
  cg.setAutoShapes(_expHintSq ? [{ orig: _expHintSq, brush: 'green' }] : []);
  const sig = _expHintSq || '';
  if (sig !== _expLastShapeSig) { _expLastShapeSig = sig; cg.redrawAll(); }
  _expRenderMoves();
}
...
// Quitte le mode solveur et remet à zéro l'état de puzzle (une seule source de
// vérité pour le teardown : les sites divergeaient sur le jeu de champs nullés).
// Ne touche PAS à la partie (_expGame) — l'appelant décide ce qu'il en fait.
function _expClearPuzzle() {
  _expPz = null; _expHintSq = null; _expFlip = false; _expSelSq = null;
...
function explorerResetBoard() {
  _expClearPuzzle();
  _expGame = new Chess(); _expUci = []; _expRedo = [];
  _expOffset = 0;
  _expSyncInput();
  _expRenderBoard();
  _expGo();
...
function explorerLoad() {
  ...
  if (raw.includes('/')) {                     // FEN
    ...
    for (const m of raw.split(/\s+/)) {
      if (!playUci(g, m)) { window.toast?.(`❌ Coup UCI invalide : ${m}`, 'ko'); return; }
      uci.push(m);
    }
...
// Le dialogue d'export (coach-explorer-export.js) lit la position courante par ce
// seul point d'accès — pas de copie de _expUci/_expGame hors de ce module.
EX.posParam = _expPosParam;
...
function _expThermalBoard(squares) {
  ...
  for (let rank = 8; rank >= 1; rank--) {
    for (let f = 0; f < 8; f++) {
      ...
      cells += `<div style="position:relative;background:${dark ? 'var(--board-dark)' : 'var(--board-light)'}">${glow}</div>`;
    }
  }
...
function _expRatingParam() {
  return EX.levels.includes('all') ? '' : `&levels=${EX.levels.join(',')}`;
}
...
function _expRenderTable() {
  ...
  const rows = _expRows.map((p, i) => {
    ...
    return `<tr data-i="${i}" tabindex="0" role="button"
      ...
      <td class="exp-td-check" onclick="event.stopPropagation()"><label class="exp-check-cell"><input type="checkbox" class="exp-check" aria-label="Sélectionner le puzzle (difficulté ${p.rating ?? 'inconnue'}, trait aux ${trait})" onchange="_expToggleSel('${escapeHtml(p.id)}', this)"></label></td>
  ...
  resEl.innerHTML = `<table class="exp-table" aria-label="Puzzles qui suivent la position">
    <thead><tr>
      <th class="exp-td-check"><label class="exp-check-cell"><input type="checkbox" id="exp-checkall" class="exp-check" aria-label="Sélectionner tous les puzzles de la page" onchange="_expToggleAll(this)"></label></th>
      <th class="exp-th-difficulty" aria-sort="${_expSort === 'rating_asc' ? 'ascending' : 'descending'}"><button type="button" class="exp-th-sort" title="Inverser le tri par difficulté" onclick="explorerToggleSort()">Difficulté ${arrow}</button></th>
      <th class="exp-th-trait">Trait</th><th>Motifs</th>
...
async function _expEnterPuzzle(id, queue, queueIdx) {
  try {
    ...
    if (!playUci(g, pd.moves[0])) throw new Error();
...
function _expPzTried(mv) {
  ...
  if (uci === expected || _expGame.in_checkmate()) {
    ...
    if (pz.idx >= pz.solution.length || _expGame.in_checkmate()) {
      ...
      playUci(_expGame, pz.solution[pz.idx]);
      pz.idx += 1;
...
async function explorerPuzzleSolution() {
  ...
  // Respecte prefers-reduced-motion : pas de délai entre les coups (rejoue d'un trait).
  const reduce = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
  while (pz.idx < pz.solution.length) {
    playUci(_expGame, pz.solution[pz.idx]);
    pz.idx += 1;
    _expRenderBoard();
    if (!reduce) await new Promise(r => setTimeout(r, 450));   // petite animation, comme l'original
  }
  pz.solved = true;
  _expPzPanel();
}
...
function _uciToKp(fen, moves) {
  ...
  if (!moves.length || !playUci(g, moves[0])) return null;   // coup d'armement
  const kpFen = g.fen();
  const line = [];
  for (const uci of moves.slice(1)) {
    const mv = playUci(g, uci);
    if (!mv) break;
    line.push(mv.san);
  }
...
// Le dialogue « Dossier de puzzles » (export PGN) vit dans coach-explorer-export.js
// (sous-fonctionnalité autonome, seul lien = EX.nfen/EX.levels/EX.posParam).

Object.assign(window, {
  renderExplorer, explorerLoad, explorerCreatePacket,
