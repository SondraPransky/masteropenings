...
import { extractAllLines, splitPgnGames, replacePgnGame, figurineTitle, pgnStartFen } from './core.js';
import { _buildDrillTree, _diffAgainstCoach, _editorTreeToDrillTree, buildTreeModule, chapterCount, chapterPgn, applyChapTitles } from './tree.js';
...
// Choix du chapitre a editer (module multi-chapitres). Modal construit a la volee,
// meme squelette que modal-create-choice ; la selection rappelle openPgnEditor(i, idx).
function _edOpenChapterPicker(i) {
  const d = G.drills[i];
  let m = document.getElementById('modal-chapter-pick');
  if (!m) {
    m = document.createElement('div');
    m.id = 'modal-chapter-pick';
    m.className = 'overlay';
    document.body.appendChild(m);
  }
  const rows = (d.sessions || []).map((s, idx) =>
    `<button class="create-choice" onclick="closeModal('modal-chapter-pick');openPgnEditor(${i}, ${idx})">
       <span class="create-choice-ico"><i class="ti ti-book-2" aria-hidden="true"></i></span>
       <span class="create-choice-txt"><b>Chapitre ${idx + 1}</b><small>${figurineTitle(escapeHtml(s.label || ''))}</small></span>
     </button>`).join('');
  m.innerHTML = `<div class="modal" style="max-width:460px;width:96vw;padding:24px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
      <div class="modal-title" style="margin:0">Quel chapitre modifier ?</div>
      <button class="btn btn-ghost btn-sm" onclick="closeModal('modal-chapter-pick')" aria-label="Fermer">✕</button>
    </div>
    <div style="font-size:.82rem;color:var(--dim);margin-bottom:18px">Ce module vient d'un fichier à ${d.sessions.length} leçons — chaque chapitre s'édite séparément.</div>
    <div class="create-choices">${rows}</div>
  </div>`;
  m.classList.add('on');
}

function openPgnEditor(i, chapIdx, opts) {
  const d = G.drills[i];
  // Module a CHAPITRES : on edite UN chapitre a la fois (sa partie du PGN).
  // Charger d.pgn entier fusionnerait les parties, et la sauvegarde detruirait
  // les autres chapitres (pgn + sessions reecrits en partie unique).
  if (chapIdx == null && chapterCount(d) > 1) { _edOpenChapterPicker(i); return; }
  _E.target = 'drill';
  _E.drillIdx = i;
  _E.chapIdx = chapIdx != null ? chapIdx : null;   // index de SESSION (chapitre) en cours d'edition
  const sess = _E.chapIdx != null ? d.sessions?.[_E.chapIdx] : d.sessions?.[0];
  _E.startFen = sess?.startFen || new Chess().fen();
  _E.flipped  = (d.side === 'b');
  _E.sel = _E.lastFrom = _E.lastTo = null;
  // Préférer le PGN (contient toutes les variantes) aux sessions filtrées par varmode
  const srcPgn = _E.chapIdx != null ? chapterPgn(d, _E.chapIdx) : d.pgn;
  if (srcPgn) {
    try { _E.root = pgnToEditorTree(srcPgn, _E.startFen); }
  ...
  // Le camp reste MODIFIABLE en édition (il l'était pas : « déjà défini → inutile »).
  // Faux depuis l'import : un module importé prend le camp choisi à l'import (et
  // l'import multi-fichiers le donne à tout le lot) — le corriger après coup est un
  // besoin réel. `saveEditorDrill` reconstruit l'arbre avec le nouveau camp.
  document.getElementById('editor-side').style.display = '';
  document.getElementById('editor-level').value = d.level || 'Intermédiaire';
  document.getElementById('editor-comment').value = '';
  _edSetDock(opts.dock || null);            // écran unique des modules (option A) ou modal classique
  editorDensity(opts.density || 'annot');
...
// ── Ancrage de l'éditeur DANS une section (03/08 : « en finir avec les 2 vues ») ──
// L'éditeur reste UN SEUL élément DOM ; ancré, il est déplacé dans un hôte de section
// (classe .editor-docked = plus une overlay) puis restitué à <body> à la fermeture.
// getElementById continue de marcher où qu'il vive — aucun autre code ne change.
function _edSetDock(dockId) {
  const modal = document.getElementById('modal-pgn-editor');
  if (dockId) {
    const h = document.getElementById(dockId);
    if (h) {
      // Changement d'hôte (ex. Modules → Parties) : l'ancien hôte restaure sa liste,
      // sinon il resterait affiché avec un dock VIDE.
      if (_E.docked && modal.parentElement !== h) { const cb = _E.onUndock; _E.onUndock = null; cb?.(); }
      h.appendChild(modal); modal.classList.add('editor-docked'); _E.docked = true; return;
    }
  }
  if (_E.docked) {
    document.body.appendChild(modal); modal.classList.remove('editor-docked'); _E.docked = false;
    const cb = _E.onUndock; _E.onUndock = null; cb?.();   // la section hôte restaure sa liste
  }
}

// Densité : 'lecture' (outils masqués, plateau inerte — on regarde la partie) ou
// 'annot' (l'éditeur complet). La lecture est le défaut d'une partie ouverte depuis
// Parties ; ✎ bascule. Les gardes d'entrée du plateau lisent _E.density.
function editorDensity(d) {
  _E.density = d === 'lecture' ? 'lecture' : 'annot';
  document.getElementById('modal-pgn-editor').classList.toggle('editor-lecture', _E.density === 'lecture');
}
...
function closeEditorModal() {
  ...
  _E.chapIdx = null;   // sortie du mode « edition d'un chapitre »
...
// Fusionne l'arbre `b` dans `a` (même position) : greffe récursive par SAN —
// c'est ce qui recolle les chapitres d'un module B2 en un seul arbre d'édition.
function _mergeEditorTrees(a, b) {
  b.children.forEach(cb => {
    const ca = a.children.find(x => x.san === cb.san);
    if (ca) _mergeEditorTrees(ca, cb);
    else a.children.push(cb);
  });
}
...
function openStudentLayerEditor(drill, role) {
  ...
  // ⚠ FIX 03/08 : l'ancien code lisait `sessions[].pgn` — un champ qui n'existe
  // PLUS depuis les modules à chapitres (B2, 21/07 : sessions = {label, startFen,
  // gameIdx}) → l'éditeur « Mes lignes » s'ouvrait TOUJOURS vide. Le PGN vit dans
  // d.pgn (multi-parties) : on parse chaque chapitre et on FUSIONNE les arbres qui
  // partagent la position de départ (les chapitres se chevauchent — mesure B2).
  // Un chapitre à racine disjointe n'est pas montré ici (rare) ; le diff reste
  // additif, il ne peut pas perdre les coups du coach.
  let root = null;
  for (const gpgn of splitPgnGames(d.pgn || '')) {
    if (!gpgn || !gpgn.trim()) continue;
    const gFen = pgnStartFen(gpgn) || new Chess().fen();
    let t = null;
    try { t = pgnToEditorTree(gpgn, gFen); } catch (e) { t = null; }
    if (!t) continue;
    if (!root) { root = t; _E.startFen = gFen; }
    else if (t.fenAfter === root.fenAfter) _mergeEditorTrees(root, t);
  }
  _E.root = root || { san:null, fenBefore:null, fenAfter:_E.startFen, comment:'', children:[] };
...
function openReviewEditor(pgn, opts) {
  ...
  if (sv) sv.innerHTML = _E.reviewRole === 'coach'
    ? '<i class="ti ti-device-floppy" aria-hidden="true"></i> Enregistrer'
    : '<i class="ti ti-device-floppy" aria-hidden="true"></i> Enregistrer';
  _edSetDock(opts.dock || null);            // ancré dans une section, ou overlay classique
  editorDensity(opts.density || 'annot');   // 'lecture' = outils masqués, plateau inerte
  document.getElementById('modal-pgn-editor').classList.add('on');
  _editorRefresh();
}
...
const _E_SQ_MIN = 28, _E_SQ_MAX = 64;   // 64 = ~512px, la partie prime → plateau plus grand
const _E_NOTATION_MIN = 360;            // largeur mini gardee a la notation en cote-a-cote
function _eResize() {
  const vw = window.innerWidth;
  // Repli par paliers si le corps n'est pas encore reflowe (clientWidth 0 a l'ouverture).
  let sq = vw < 500 ? 34 : vw < 750 ? 42 : _E_SQ_MAX;
  const col  = document.getElementById('editor-board-col');
  const body = document.getElementById('editor-body');
  if (col && body && body.clientWidth > 0) {
    const cs = getComputedStyle(col);
    const padH  = (parseFloat(cs.paddingLeft) || 0) + (parseFloat(cs.paddingRight) || 0);
    const ranks = 16;   // gouttiere des chiffres de rangee, hors du plateau
    let avail;
    // ⚠ La colonne est `flex:0 0 auto` (dimensionnee par son contenu) : deriver la
    // taille du plateau de `col.clientWidth` etait CIRCULAIRE (le plateau restait a
    // sa taille courante, plafonnee a 48) → un grand vide a droite. On derive
    // desormais du CORPS (largeur fixe du modal), en reservant sa place a la notation.
    if (vw < 860) {
      // Empile : le plateau remplit la colonne pleine largeur, borne par la fenetre.
      avail = Math.min(col.clientWidth, vw) - padH - ranks;
    } else {
      // Cote-a-cote : plateau = corps - notation - marges, plafonne.
      avail = body.clientWidth - _E_NOTATION_MIN - padH - ranks;
    }
...
function editorDragStart(e, sq) {
  if (_E.density === 'lecture') return;   // lecture : le plateau est inerte (✎ pour annoter)
...
// « Insérer en citation » (depuis la visionneuse du panneau Référence) : greffe une
// suite de coups en sous-variante au nœud courant, citation en commentaire sur le 1er.
// La partie passe par la position courante par construction (référence POUR elle).
function editorInsertCitation(sans, cite) {
  if (!sans || !sans.length || !_E.node) { toast('Rien à insérer', 'ko'); return; }
  const startPath = _E.path.slice();
  const g = new Chess(_E.node.fenAfter);
  let firstIdx = -1;
  for (let i = 0; i < sans.length; i++) {
    ...
    if (!mv) break;
    if (i === 0) {
      const ex = _E.node.children.findIndex(c => c.san === mv.san);
      firstIdx = ex >= 0 ? ex : _E.node.children.length;   // où le 1er coup va atterrir
    }
    editorApplyMove(mv, g.fen());        // ajoute (ou entre dans) l'enfant + navigue
  }
  if (cite && firstIdx >= 0) {           // citation sur le 1er coup greffé, au point de branche
    let branch = _E.root; for (const idx of startPath) branch = branch.children[idx];
    const first = branch.children[firstIdx];
    if (first) first.comment = (first.comment ? first.comment + ' ' : '') + '(' + cite + ')';
  }
  editorGoPath(startPath);               // revient au point de branche + rafraîchit la notation
  toast('✓ Citation insérée — pense à enregistrer le module', 'ok');
}
...
function editorGoPath(path) {
  ...
  _editorRenderRef();   // le panneau Référence SUIT la position courante (debounce + cache)
}

// ── Onglets du panneau de droite : Notation | Référence (03/08) ────────────────
// La Référence ne se rend (et ne fetch) que si son onglet est actif.
let _edSideTab = 'notation';
function editorSideTab(t) {
  _edSideTab = t === 'ref' ? 'ref' : 'notation';
  const notation = _edSideTab === 'notation';
  const bn = document.getElementById('editor-tab-notation'), br = document.getElementById('editor-tab-ref');
  if (bn) { bn.classList.toggle('on', notation); bn.setAttribute('aria-selected', String(notation)); }
  if (br) { br.classList.toggle('on', !notation); br.setAttribute('aria-selected', String(!notation)); }
  const nt = document.getElementById('editor-notation'); if (nt) nt.style.display = notation ? '' : 'none';
  const rf = document.getElementById('editor-ref'); if (rf) rf.style.display = notation ? 'none' : '';
  if (!notation) _editorRenderRef();
}
...
function _editorRefresh() {
  editorSideTab('notation');         // chaque ouverture repart sur la notation
  _eResize(); renderEditorBoard(); renderEditorNotation(); renderEditorNagBar();
  _ensureEditorArrowHandlers(); _syncEditorCommentField();
}

// Panneau Référence (base Maîtres Lichess) synchronisé sur la position courante.
// Débouncé : une navigation rapide dans l'arbre ne doit pas mitrailler l'API
// (le cache par position + l'anti-course de reference.js couvrent le reste).
// Clic sur un coup = on le JOUE (grill 29/07) → il entre dans le répertoire.
let _refTimer;
function _editorRenderRef() {
  const host = document.getElementById('editor-ref');
  if (!host || !_E.node) return;
  if (_edSideTab !== 'ref') return;   // onglet inactif : ni rendu ni fetch
  clearTimeout(_refTimer);
  const fen = _E.node.fenAfter;
  _refTimer = setTimeout(() => {
    window.renderReferencePanel?.({
      hostId: 'editor-ref',
      fen,
      flip: !!_E.flipped,
      onMove: (san) => {
        const g = new Chess(_E.node.fenAfter);
        const mv = g.move(san);
        if (mv) { _E.lastFrom = mv.from; _E.lastTo = mv.to; _E.sel = null; editorApplyMove(mv, g.fen()); }
      },
      // « Insérer en citation » : greffe les coups de la partie (position → coupe) en
      // sous-variante au nœud courant, citation en commentaire sur le 1er coup.
      onInsert: (sans, cite) => editorInsertCitation(sans, cite),
    });
  }, 220);
}
...
function renderEditorNotation() {
  ...
  function nodeHTML(node, path, forceNum) {
    ...
    // Coup courant : classe .ed-cur (encre par thème) — l'inline color:#111 échouait AA en clair.
    const s=isCur?'':
      (_authorStyle(main) ? _authorStyle(main) :
       isOnPath?'color:var(--text);font-weight:600':'color:var(--text-2)');
    h+=`<span onclick="editorGoPath(${JSON.stringify(mainPath)})"${isCur?' class="ed-cur"':''} style="cursor:pointer;${s};padding:1px 4px;border-radius:3px">${_authorMark(main)}${fig(main.san)}${nagGlyphs(main)}</span>`;
    h+=_cmtSpans(main);
    ...
    vars.forEach((v,vi) => {
      ...
      h+=`<span onclick="editorGoPath(${JSON.stringify(vPath)})"${isCurV?' class="ed-cur"':''} style="cursor:pointer;${vs};padding:1px 4px;border-radius:3px">${_authorMark(v)}${fig(v.san)}${nagGlyphs(v)}</span>`;
      h+=_cmtSpans(v);
...
// Sauvegarde d'UN CHAPITRE d'un module multi-chapitres : seule sa partie du PGN
// est remplacee, puis le module (arbre fusionne + sessions) est rebati depuis le
// PGN complet par buildTreeModule — la meme fabrique que l'import, donc les
// labels de chapitre (en-tetes White/Black) et les startFen restent coherents.
function _saveEditorChapter() {
  const d = G.drills[_E.drillIdx];
  if (!d) { toast('❌ Module introuvable', 'ko'); return; }
  const name = (document.getElementById('editor-drill-name').value || '').trim() || d.name;
  const side =  document.getElementById('editor-side').value || d.side || 'w';
  const level = document.getElementById('editor-level').value || d.level;

  // Movetext edite (sans les en-tetes [SetUp]/[FEN] qu'emet editorTreeToPGN :
  // le chunk garde SES en-tetes d'origine — c'est sur eux que reposent la
  // decoupe splitPgnGames ([Event]) et les labels de chapitre (White/Black)).
  const edited = editorTreeToPGN(_E.root, _E.startFen);
  const movetext = edited.replace(/^\s*(?:\[[^\]]*\]\s*)+/, '').trim();
  if (!movetext || movetext === '*') { toast('⚠ Joue au moins un coup sur l\'échiquier', 'ko'); return; }

  const gi   = d.sessions?.[_E.chapIdx]?.gameIdx ?? _E.chapIdx;
  ...
  const rebuilt = buildTreeModule({
    id: d.id, name, pgn: newPgn, side, level,
    deadline: d.deadline, hideComments: d.hideComments,
  });
  if (!rebuilt) { toast('❌ PGN reconstruit invalide — rien n\'a été modifié', 'ko'); return; }
  // Les titres de chapitres édités par le coach priment sur les en-têtes PGN
  // (sans ça, la reconstruction les reperdrait silencieusement).
  applyChapTitles(rebuilt.sessions, d.chapTitles);

  d.name = name; d.side = side; d.level = level;
  d.pgn = newPgn; d.tree = rebuilt.tree; d.sessions = rebuilt.sessions;
  d.updatedAt = Date.now();

  const chapNum = _E.chapIdx + 1;      // AVANT closeEditorModal, qui remet chapIdx a null
  ...
  toast(`✓ Chapitre ${chapNum} mis à jour`, 'ok');
}
...
function saveEditorDrill() {
  ...
  if (_E.chapIdx != null)     { _saveEditorChapter(); return; } // module a chapitres → seule SA partie du PGN est remplacee
  const pgn  = editorTreeToPGN(_E.root, _E.startFen);
...
Object.assign(window, { _E, openPgnEditor, openPgnEditorNew, closeEditorModal, _eResize, editorSideTab, editorDensity, editorClickSqLogic, _editorShapesSVG, _ensureEditorArrowHandlers, editorToggleShape, editorClearShapes, renderEditorBoard, editorDragStart, editorDrop, editorDragEnd, editorTouchStart, editorClickSq, editorApplyMove, editorGoPath, editorPrev, editorNext, flipEditorBoard, editorSaveComment, editorValidateComment, editorDeleteNode, editorPromoteMain, editorToggleNag, editorClearNags, renderEditorNagBar, renderEditorNotation, saveEditorDrill, openGameEditor, openReviewEditor, openStudentLayerEditor });
