...
const COACH_COL = '#7c3aed';   // violet — coups/variantes ajoutés par le coach (P1.4/P1.5)
...
function openPgnEditor(i) {
  ...
  _eResize(); renderEditorBoard(); renderEditorNotation(); renderEditorNagBar(); _ensureEditorArrowHandlers(); _syncEditorCommentField();
}
...
function closeEditorModal() {
  ...
  _E.target = 'drill'; _E.reviewGameId = null; _E.reviewRole = null;
  ...
  const sv = document.getElementById('editor-save-btn');    if (sv) sv.textContent = '💾 Enregistrer';
}
...
function openReviewEditor(pgn, opts) {
  ...
  const who = (opts.white || '?') + ' – ' + (opts.black || '?');
  if (t) t.textContent = _E.reviewRole === 'coach'
    ? `🧑‍🏫 Revue — ${who} · ajoute variantes & commentaires`
    : `✍️ ${who} · clique un coup pour l'annoter (revue du coach en violet)`;
  const sv = document.getElementById('editor-save-btn');
  if (sv) sv.textContent = _E.reviewRole === 'coach' ? '💾 Enregistrer la revue' : '💾 Enregistrer';
  document.getElementById('modal-pgn-editor').classList.add('on');
  _eResize(); renderEditorBoard(); renderEditorNotation(); renderEditorNagBar(); _ensureEditorArrowHandlers(); _syncEditorCommentField();
}
...
function editorApplyMove(mv, fenAfter) {
  ...
  // En revue coach, tout coup ajouté est marqué (couche additive, rendu en couleur).
...
function editorValidateComment() {
  ...
  _E.node[_commentKey()] = ta.value;
  renderEditorNotation();
  ta.blur();
...
function renderEditorNotation() {
  ...
  function nodeHTML(node, path, forceNum) {
    ...
    const s=isCur?'background:var(--cyan);color:#111;padding:1px 6px;border-radius:3px;font-weight:700':
      main.author==='coach'?`color:${COACH_COL};font-weight:700`:
    ...
    if (main.comment) h+=` <span style="color:var(--dim);font-style:italic;font-family:var(--font-ui);font-size:.8rem">${escapeHtml(main.comment)}</span> `;
    if (main.coachComment) h+=` <span style="color:${COACH_COL};font-style:italic;font-family:var(--font-ui);font-size:.8rem"><i class="ti ti-school" aria-hidden="true"></i> ${escapeHtml(main.coachComment)}</span> `;
    ...
    vars.forEach((v,vi) => {
      ...
      const vs=isCurV?'background:var(--cyan);color:#111;padding:1px 6px;border-radius:3px;font-weight:700':
        v.author==='coach'?`color:${COACH_COL};font-weight:700`:
      ...
      if (v.comment) h+=` <span style="color:var(--dim);font-style:italic;font-family:var(--font-ui);font-size:.8rem">${escapeHtml(v.comment)}</span> `;
      if (v.coachComment) h+=` <span style="color:${COACH_COL};font-style:italic;font-family:var(--font-ui);font-size:.8rem"><i class="ti ti-school" aria-hidden="true"></i> ${escapeHtml(v.coachComment)}</span> `;
