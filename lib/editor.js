...
function openPgnEditor(i, chapIdx) {
  const d = G.drills[i];
...
function _edSetDock(dockId) {
  ...
  if (dockId) {
    ...
    if (h) { h.appendChild(modal); modal.classList.add('editor-docked'); _E.docked = true; return; }
  }
  if (_E.docked) { document.body.appendChild(modal); modal.classList.remove('editor-docked'); _E.docked = false; }
}
