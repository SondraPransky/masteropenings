...
function openStudentLayerEditor(drill, role) {
  ...
  const who = d._overlayBy?.student ? ` de ${d._overlayBy.student}` : '';
  if (t) t.textContent = _E.reviewRole === 'coach'
    ? `🧑‍🏫 ${d.name} — copie${who} · ses lignes en bleu, tes réponses en violet`
    : `✎ ${d.name} — ajoute tes lignes · celles du coach restent intactes`;
  const sv = document.getElementById('editor-save-btn');
  if (sv) {
    sv.textContent = _E.reviewRole === 'coach' ? '💾 Enregistrer ma réponse' : '💾 Enregistrer mes lignes';
    sv.style.display = '';
  }
  document.getElementById('modal-pgn-editor').classList.add('on');
  _editorRefresh();
