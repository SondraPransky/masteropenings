...
let _pending = null;        // la requête UCI EN VOL : { onLine, fail } — une seule à la fois
let _chain = Promise.resolve();   // sérialise les analyses
const _MAX_PLIES = 200;     // garde-fou : au-delà, une « partie » est un collage
const SF_DEPTH = 12;        // profondeur par défaut (la file d'analyse la partage)

// ⚠ TOUTE panne passe par ici. Un worker qui a raté une position CONTINUE de
// chercher : sa réponse tardive serait attribuée à la position suivante (bestmove
// décalé → analyse fausse, persistée pour toujours dans games.extra.analysis).
// On ne récupère donc jamais un moteur en vol : on le tue et on reboote en lazy.
function _sfKill(err) {
  const p = _pending; _pending = null;
  if (_worker) { try { _worker.terminate(); } catch (e) {} _worker = null; }
  p?.fail(err || new Error('stockfish : moteur arrêté'));
}
...
function _sfBoot() {
  return new Promise((resolve, reject) => {
    ...
    w.onmessage = (ev) => {
      if (!String(ev.data).startsWith('uciok')) return;
      clearTimeout(t);
      // Régime permanent : UN SEUL handler pour toute la vie du worker, qui route
      // vers la requête en vol. (Le permuter à chaque position laissait une
      // réponse tardive tomber dans l'écoute de la position d'après.)
      w.onmessage = (m) => _pending?.onLine(String(m.data));
      w.onerror = (e) => _sfKill(e.error || new Error('stockfish : worker en erreur'));
      resolve(w);
    };
    w.postMessage('uci');
  });
...
function _sfEval(w, fen, depth) {
  return new Promise((resolve, reject) => {
    let last = null;
    // Le timeout TUE le moteur (cf. _sfKill) : il ne suffit pas d'arrêter d'écouter.
    const t = setTimeout(() => _sfKill(new Error('stockfish : position sans bestmove')), 30000);
    _pending = {
...
function sfAnalyzeGame(pgn, /** @type {{depth?: number, onStep?: Function}} */ { depth = SF_DEPTH, onStep } = {}) {
  ...
  const run = async () => {
    ...
    // `w` reste valide tant qu'aucune position n'échoue : le premier échec tue le
    // moteur ET rejette, donc la boucle s'arrête ici — on ne parle jamais à un
    // worker terminé, et la partie suivante rebootera un moteur propre.
...
// Libère le moteur quand la file a fini : 7 Mo de WASM + un thread n'ont aucune
// raison de rester résidents dans l'onglet du coach entre deux lots. Le prochain
// `sfAnalyzeGame` reboote en lazy. No-op si une analyse est en vol.
function sfRelease() { if (!_pending) _sfKill(); }

export { sfAnalyzeGame, sfAvailable, sfRelease, SF_DEPTH };
