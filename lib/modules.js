...
function renderCoachOnboarding() {
  ...
  const steps = [
    ...
    { done: nbModules > 0, label: 'Créez votre premier module',
      cta: `<button class="btn btn-gold btn-sm" onclick="openCreateChoice()">Créer</button>` },
...
function renderDrillList() {
  ...
  if (!n) {
    ...
    grid.innerHTML = `<div class="mcard-empty">
      ...
      ${onbActive ? '' : `<button class="btn btn-gold" onclick="openCreateChoice()">+ Créer mon premier module</button>`}
    </div>`;
    return;
  }
  ...
  grid.innerHTML = G.drills.map((d,i) => {
    ...
    const dueBanner = dueCount>0
      ? `<div class="mcard-due-banner" onclick="event.stopPropagation();reviserDrill(${i})"><i class="ti ti-rotate" aria-hidden="true"></i> ${dueCount} coup${dueCount>1?'s':''} à réviser</div>`
      : '';
    ...
    const badges = [
      ...
      d.demo    ? `<span class="badge" style="background:var(--gold-dim);color:var(--gold)"><i class="ti ti-flask" aria-hidden="true"></i> Démo</span>` : '',
      d.hideComments ? `<span class="badge" style="background:var(--surf2);color:var(--dim)"><i class="ti ti-volume-off" aria-hidden="true"></i> Confirmé</span>` : '',
    ...
    const editorBtn = d.isExercise
      ? `<button class="btn btn-ghost btn-sm" onclick="openExercisePacket(${d.id})" title="Modifier les exercices"><i class="ti ti-puzzle" aria-hidden="true"></i></button>`
      : `<button class="btn btn-ghost btn-sm" onclick="openPgnEditor(${i})" title="Éditeur sur échiquier"><i class="ti ti-edit" aria-hidden="true"></i></button>`;
    const maiaBtn = d.isExercise ? '' : `<button class="btn btn-ghost btn-sm" onclick="playVsMaia(${i})" title="Jouer contre Maia"><i class="ti ti-robot" aria-hidden="true"></i></button>`;
    ...
    return `<div class="mcard">
      ...
      <div class="mcard-footer">
        <button class="btn btn-gold btn-sm" onclick="launchDrill(${i})"><i class="ti ti-player-play" aria-hidden="true"></i> Jouer</button>
        ...
        ${maiaBtn}
        ${editorBtn}
        <button class="btn btn-ghost btn-sm" onclick="deleteDrill(${d.id})" title="Supprimer"><i class="ti ti-trash" aria-hidden="true"></i></button>
      </div>
    </div>`;
  }).join('');
}
...
function renderClassList() {
  ...
  el.innerHTML = G.classes.map(cls => {
    ...
    const modNames = (cls.moduleIds || []).map(id => {
      ...
      return dl ? `${nm} · <i class="ti ti-calendar" aria-hidden="true"></i> ${dl}` : nm;
    });
    ...
    return `<div style="padding:10px 12px;background:var(--surf2);border:1px solid var(--border);border-radius:var(--r);margin-bottom:8px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="flex:1;min-width:0">
          ...
          ${modNames.length ? `<div style="font-size:.7rem;color:var(--cyan);margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><i class="ti ti-stack-2" aria-hidden="true"></i> ${modNames.map(escapeHtml).join(', ')}</div>` : ''}
          ${stuList.length ? `<div style="font-size:.7rem;color:var(--dim);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><i class="ti ti-user" aria-hidden="true"></i> ${stuList.slice(0,4).map(escapeHtml).join(', ')}${stuList.length>4?' +'+(stuList.length-4):''}</div>` : ''}
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0">
          <button class="btn btn-ghost btn-sm" onclick="openEditClass(${cls.id})" title="Modifier"><i class="ti ti-edit" aria-hidden="true"></i></button>
