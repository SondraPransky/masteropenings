...
function renderDrillList() {
  ...
  grid.innerHTML = list.map(({ d, i, due }) => {
    ...
    const dueBanner = due>0
      ? `<div class="mcard-due-banner" onclick="event.stopPropagation();reviserDrill(${i})"><i class="ti ti-rotate" aria-hidden="true"></i> ${due} coup${due>1?'s':''} à réviser</div>`
      : '';
    ...
    return `<div class="cls-card" role="button" tabindex="0" onclick="openClassDetail(${cls.id})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openClassDetail(${cls.id})}" aria-label="Ouvrir la classe ${escapeHtml(cls.individual ? cls.name.replace(/^👤\s*/,'') : cls.name)}">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        ...
        <div style="display:flex;gap:6px;flex-shrink:0">
          <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openEditClass(${cls.id})" title="Modifier"><i class="ti ti-edit" aria-hidden="true"></i></button>
          <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();deleteClass(${cls.id})" title="Supprimer"><i class="ti ti-trash" aria-hidden="true"></i></button>
        </div>
      </div>
    </div>`;
  }).join('');
