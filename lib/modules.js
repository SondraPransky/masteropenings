...
function renderCoachOnboarding() {
  ...
  const steps = [
    ...
    { done: nbClasses > 0, label: 'Créez une classe et ajoutez vos élèves',
      cta: `<button class="btn btn-primary btn-sm" onclick="switchCoachSection('eleves');openClassForm()">Créer une classe</button>` }
  ];
...
function shareDrill(i) {
  const d = G.drills[i];
  if (!d) return;
  switchCoachSection('eleves');  // affiche l'onglet Élèves (la modale s'ouvre par-dessus)
