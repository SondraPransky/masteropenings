...
function _pgnBatchStart(reads) {
  _pgnBatch = reads.map(({ fname, pgn }) => {
    ...
    let positions = 0, error = null;
    try {
      const lines = extractAllLines(pgn);
      positions = Object.keys(_buildDrillTree(lines, 'w')).length;
      if (!positions) error = 'aucun coup extractible';
    } catch (e) { error = e.message || 'PGN invalide'; }
    return { fname, pgn, name, side: 'w', positions, error };
  });
  _renderPgnBatch();
}
...
function _renderPgnBatch() {
  ...
  const rows = _pgnBatch.map((r, i) => r.error
    ...
    : `<div class="pgnb-row"><span class="pgnb-file" title="${escapeHtml(r.fname)}">${escapeHtml(r.fname)}</span>
         ...
         <span class="pgnb-pos">${r.positions} pos.</span></div>`).join('');
  el.innerHTML = `
    <div class="pgnb-head">${_pgnBatch.length} fichiers sélectionnés — un module par fichier.
    ...
    <div class="pgnb-actions">
      ...
      <button class="btn btn-primary btn-sm" onclick="importPgnBatch()" ${valid ? '' : 'disabled'}>
        <i class="ti ti-plus" aria-hidden="true"></i> Créer ${valid} module${valid > 1 ? 's' : ''}</button>
    </div>`;
  el.style.display = '';
...
function importPgnBatch() {
  ...
  _pgnBatch.forEach((r, i) => {
    ...
    let allLines;
    try { allLines = extractAllLines(r.pgn); } catch (e) { failed++; return; }
    const tree = _buildDrillTree(allLines, r.side);
    if (!Object.keys(tree).length) { failed++; return; }
    const d = {
      id: baseId + i,
      name: r.name, level, side: r.side, pgn: r.pgn,
      mode: 'line', varmode: 'tree', tree,
      sessions: [{ label: 'Arbre complet', startFen: new Chess().fen(), moves: [], kps: [] }],
