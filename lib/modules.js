...
// ⚠ On mémoïse l'ANALYSE (coûteuse), jamais le HTML : `fig` arrive par le pont
// window et n'est pas encore posé au tout premier rendu — un HTML mis en cache à
// ce moment-là resterait sans figurines pour toute la session.
...
function _openingSans(d, plies) {
  ...
  _openLineCache.set(key, sans);
  return sans;
}

// Mise en forme de la ligne : numéros de coup + figurines. Recalculée à chaque
// rendu (quelques concaténations) pour ne jamais figer un état du pont window.
function _openingLine(d, plies) {
  ...
  sans.forEach((san, k) => {
    ...
    out += window.fig ? window.fig(escapeHtml(san)) : escapeHtml(san);
    plain += san;
...
function renderDrillList() {
  ...
  const _rowHTML = ({ d, i, due }) => {
    ...
    return `<div class="mrow">
      ...
      <div class="mrow-main">
        <div class="mrow-name">${escapeHtml(d.name)}</div>
        ${flags ? `<div class="mrow-flags">${flags}</div>` : ''}
      </div>
      <div class="mrow-line" title="${escapeHtml(_openingLine(d, 10).plain)}">${_openingLine(d, 6).html}</div>
      ...
      <div class="mrow-acts">
        ...
        <button class="btn btn-blue btn-sm" onclick="shareDrill(${i})" title="Assigner ce module à des élèves"><i class="ti ti-share" aria-hidden="true"></i> Assigner</button>
