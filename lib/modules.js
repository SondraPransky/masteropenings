...
import { extractAllLines, pgnHeader, pgnStartFen, splitPgnGames } from './core.js';
import { _buildDrillTree, isPlayerMove, buildTreeModule, gameModuleName } from './tree.js';
...
function previewDrill() {
  ...
  el.innerHTML = `<div style="display:flex;gap:16px;margin-bottom:8px;font-size:.82rem">
    ...
    <span style="color:var(--dim)">${playerPos} à jouer · ${lines.length} variante${lines.length>1?'s':''}${games.length>1?` · ${games.length} parties → ${games.length} modules`:''}</span>
  </div>${rows.join('')}`;
}
...
function _renderPgnBatch() {
  ...
  const rows = _pgnBatch.map((r, i) => r.error
    ...
    : `<div class="pgnb-row"><span class="pgnb-file" title="${escapeHtml(r.fname)}">${escapeHtml(r.fname)}</span>
         ...
         <span class="pgnb-pos">${r.games > 1 ? `${r.games} parties · ` : ''}${r.positions} pos.</span></div>`).join('');
  const totalMods = _pgnBatch.reduce((a, r) => a + (r.error ? 0 : Math.max(1, r.games)), 0);
  el.innerHTML = `
    <div class="pgnb-head">${_pgnBatch.length} fichiers sélectionnés — ${totalMods} module${totalMods > 1 ? 's' : ''} (un par partie du PGN).
...
function importPgnBatch() {
  ...
  let created = 0, failed = 0, seq = 0;
  const baseId = Date.now();   // ⚠ Date.now() seul se répéterait dans la boucle
  _pgnBatch.forEach(r => {
    if (r.error) { failed++; return; }
    // Un fichier peut contenir plusieurs parties → un module par partie.
    const games = splitPgnGames(r.pgn);
    const chunks = games.length ? games : [r.pgn];
    let madeHere = 0;
    chunks.forEach((chunk, gi) => {
      ...
      try {
        d = buildTreeModule({
          id: baseId + (seq++),
          name: chunks.length > 1 ? gameModuleName(chunk, r.name, gi) : r.name,
          pgn: chunk, side: r.side, level, deadline, hideComments,
        });
      } catch (e) { return; }
      if (!d) return;
      G.drills.push(d);
...
function importDrill() {
  ...
  // Un PGN peut contenir PLUSIEURS parties (fichier de lecon exporte de ChessBase) :
  // chacune a sa propre position de depart, donc chacune donne son propre module.
  const games = splitPgnGames(pgn);
  const chunks = games.length ? games : [pgn];
  const baseId = Date.now();   // ⚠ Date.now() seul se repeterait dans la boucle
  const made = [];
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      d = buildTreeModule({
        id: baseId + i,
        name: chunks.length > 1 ? gameModuleName(chunks[i], baseName, i) : baseName,
        pgn: chunks[i], side, level, deadline, hideComments,
      });
    } catch (e) { toast('❌ PGN invalide : ' + e.message, 'ko'); return; }
    if (d) made.push(d);
  }
  if (!made.length) { toast('❌ Aucune ligne jouable', 'ko'); return; }

  for (const d of made) { G.drills.push(d); saveModule(d); }
  ...
  closeModal('modal-create-drill');
  const pos = made.reduce((a, d) => a + Object.keys(d.tree).length, 0);
  toast(made.length > 1
    ? `✓ ${made.length} modules créés (${made.length} parties dans le PGN) — ${pos} positions indexées`
    : `✓ Module créé — ${pos} positions indexées`, 'ok');
}
