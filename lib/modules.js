...
import { extractAllLines, pgnHeader, pgnStartFen, splitPgnGames, fig, figurineTitle, pgnMainlineSans } from './core.js';
import { _buildDrillTree, isPlayerMove, buildTreeModule, chapterPgn } from './tree.js';
...
// La recherche est INSENSIBLE AUX ACCENTS (_normTxt, deja utilise par le
// sous-titre de dossier). Mesure sur le fonds reel : 8 modules sur 26 portent un
// accent dans leur nom, et le fonds melange lui-meme les graphies (dossier
// « Gambit Goring », module « Gambit Goering ») -> taper « grunfeld » ou
// « goring » ne rendait RIEN, avec l'etat vide « Aucun module ne correspond »
// qui se lit comme « je n'ai pas ce module ».
function modSearch(v)     { _modQuery = _normTxt(v || ''); renderDrillList(); }
...
// Range un module dans un GROUPE (« Répertoire Groupe 1 ») — la dimension de
// rangement voulue par la coach (29/07). Distinct du `folder` d'origine d'import :
// le groupe est libre, il devient une section du répertoire. Migration-free (extra.group).
// Depuis le menu ⋯ d'un module : ouvre la modale en mode « ranger CE module ».
function moveDrillToGroup(id) { openGroupModal(id); }

// Modale de groupe (remplace le prompt natif). null = mode « créer un groupe »
// (nom + checklist de modules) ; un id = mode « ranger CE module » (nom seul,
// datalist des groupes existants). Migration-free : ne touche que d.group.
let _groupMoveId = null;
function openGroupModal(preselectId = null) {
  _groupMoveId = preselectId != null ? String(preselectId) : null;
  const groups = [...new Set(G.drills.map(x => x.group).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'fr'));
  const dl = document.getElementById('group-name-list');
  if (dl) dl.innerHTML = groups.map(g => `<option value="${escapeHtml(g)}"></option>`).join('');
  const title = document.getElementById('group-modal-title');
  const help  = document.getElementById('group-modal-help');
  const saveBtn = document.getElementById('group-save-btn');
  const input = document.getElementById('group-name-input');
  const modRow = document.getElementById('group-modules-row');
  const list = document.getElementById('group-modules');
  if (_groupMoveId) {
    const d = G.drills.find(x => String(x.id) === _groupMoveId);
    if (title) title.innerHTML = '<i class="ti ti-folders" aria-hidden="true"></i> Ranger dans un groupe';
    if (help)  help.textContent = `Choisissez un groupe existant ou saisissez-en un nouveau pour « ${d?.name || ''} ». Laissez vide pour l'en sortir.`;
    if (saveBtn) saveBtn.innerHTML = '<i class="ti ti-folders" aria-hidden="true"></i> Ranger';
    if (input) input.value = d?.group || '';
    if (modRow) modRow.style.display = 'none';
  } else {
    if (title) title.innerHTML = '<i class="ti ti-folder-plus" aria-hidden="true"></i> Nouveau groupe';
    if (help)  help.textContent = 'Nommez le groupe et cochez les modules à y ranger. Un groupe devient une section du répertoire.';
    if (saveBtn) saveBtn.innerHTML = '<i class="ti ti-folder-plus" aria-hidden="true"></i> Créer le groupe';
    if (input) input.value = '';
    if (modRow) modRow.style.display = '';
    if (list) list.innerHTML = G.drills.length
      ? G.drills.map(d => `<label class="cls-mod-row" style="display:flex;align-items:center;gap:6px;padding:3px 0;margin:0;cursor:pointer">
          <input type="checkbox" value="${escapeHtml(String(d.id))}">
          <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${figurineTitle(escapeHtml(d.name))}${d.group ? ` <span style="color:var(--dim);font-size:.72rem">(${escapeHtml(d.group)})</span>` : ''}</span>
        </label>`).join('')
      : '<div style="padding:8px;font-size:.8rem;color:var(--dim)">Aucun module créé</div>';
  }
  document.getElementById('modal-group')?.classList.add('on');
...
function saveGroupModal() {
  ...
  if (_groupMoveId) {
    ...
    _modFolder = null;   // garde le module visible meme si un filtre de dossier etait actif
    renderDrillList();
    toast(d.group ? `✓ Rangé dans « ${d.group} »` : '✓ Sorti du groupe', 'ok');
    return;
  }
  if (!name) { toast('⚠ Donnez un nom au groupe', 'ko'); return; }
  const ids = [...document.querySelectorAll('#group-modules input[type=checkbox]:checked')].map(c => c.value);
  if (!ids.length) { toast('⚠ Cochez au moins un module', 'ko'); return; }
  let n = 0;
  ids.forEach(id => {
    const d = G.drills.find(x => String(x.id) === String(id));
    if (d) { d.group = name; d.updatedAt = Date.now(); window.saveModule?.(d); n++; }
  });
  save();
  closeModal('modal-group');
  _modFolder = null;   // le nouveau groupe doit etre visible, hors de tout filtre de dossier actif
  renderDrillList();
  toast(`✓ Groupe « ${name} » créé — ${n} module${n > 1 ? 's' : ''}`, 'ok');
}
...
function _openingLine(d, plies) {
  ...
  // Le 1er demi-coup peut être noir, et le module peut partir d'une position :
  // on numérote depuis le VRAI numéro de coup du [FEN] (même règle que _chapLine).
  const sf = (d.sessions?.[0]?.startFen || '').split(' ');
  const startBlack = sf[1] === 'b';
  let out = '', plain = '', n = parseInt(sf[5], 10) || 1;
  sans.forEach((san, k) => {
    const isWhite = startBlack ? k % 2 === 1 : k % 2 === 0;
    if (isWhite) { out += `<span class="mv-num">${n}.</span>`; plain += n + '.'; n++; }
    else if (k === 0) { out += `<span class="mv-num">${n}…</span>`; plain += n + '…'; n++; }
...
// Renommer UN chapitre depuis le répertoire. Le titre édité part dans
// sessions[k].label (affichage + persistance) ET dans la surcouche
// d.chapTitles (extra, clé gameIdx) : c'est elle qui le fait SURVIVRE aux
// reconstructions de l'arbre (sauvegarde d'un chapitre dans l'éditeur), qui
// rederivent sinon les libellés des en-têtes PGN. Le PGN, lui, n'est jamais
// réécrit — l'en-tête reste la source, le titre édité une surcouche qui prime.
function renameChapter(id, k) {
  const d = G.drills.find(x => String(x.id) === String(id));
  const s = d?.sessions?.[k];
  if (!s) return;
  const v = prompt('Nouveau titre du chapitre', s.label || '');
  if (v === null) return;                    // annulé
  const name = v.trim();
  if (!name || name === s.label) return;
  s.label = name;
  d.chapTitles = { ...(d.chapTitles || {}), [String(s.gameIdx != null ? s.gameIdx : k)]: name };
  d.updatedAt = Date.now();
  save();
  window.saveModule?.(d);
  renderDrillList();
  toast(`✓ Chapitre renommé « ${name} »`, 'ok');
}

// ── ÉCRAN UNIQUE d'un chapitre (option A, 03/08) ────────────────────────────
// Cliquer un chapitre = OUVRIR : l'éditeur ancré dans #mod-dock en densité
// LECTURE (patron de l'écran des Parties) ; ✎ « Modifier » révèle les outils.
// « Tester » (ex-« Jouer ») reste le geste séparé « comme un élève ».
let _modViewCtx = null;   // { i, k } tant que l'écran est ouvert
function _modListVisible(show) {
  ['coach-onboarding', 'mod-toolbar', 'module-cards-grid'].forEach(id => {
    const el = document.getElementById(id); if (el) el.style.display = show ? '' : 'none';
  });
  const hd = document.querySelector('#csec-modules .cs-header'); if (hd) hd.style.display = show ? '' : 'none';
  const mv = document.getElementById('mod-view'); if (mv) mv.style.display = show ? 'none' : '';
}
function _modSyncTools(on) {
  const tb = document.getElementById('mod-tools-btn');
  if (tb) {
  ...
  const sv = document.getElementById('mod-save-btn');   if (sv) sv.style.display = on ? '' : 'none';
  const rn = document.getElementById('mod-rename-btn'); if (rn) rn.style.display = on ? '' : 'none';
}
function _modViewTitle() {
  const { i, k } = _modViewCtx || {}; const d = G.drills[i]; if (!d) return;
  const t = document.getElementById('mod-view-title'); if (!t) return;
  const lbl = d.sessions?.[k]?.label;
  const chap = (lbl && (d.sessions?.length || 1) > 1) ? ` · ${figurineTitle(escapeHtml(lbl))}` : '';
  t.innerHTML = `<i class="ti ti-book-2" aria-hidden="true"></i> ${figurineTitle(escapeHtml(d.name))}${chap}`;
}
function openModuleView(i, k) {
  const d = G.drills[i]; if (!d) return;
  _modViewCtx = { i, k: k || 0 };
  _modListVisible(false);
  _modViewTitle();
  _modSyncTools(false);
  window.openPgnEditor?.(i, k || 0, { dock: 'mod-dock', density: 'lecture' });
  if (window._E) window._E.onUndock = _modViewRestore;   // restitution → la liste revient
}
function _modViewRestore() { _modViewCtx = null; _modListVisible(true); }
function closeModuleView() {
  if (window._E?.docked) window.closeEditorModal?.();   // déclenche onUndock → restore
  else _modViewRestore();
  renderDrillList();
}
function modToggleTools() {
  const on = window._E?.density === 'lecture';   // on VA vers les outils ?
  window.editorDensity?.(on ? 'annot' : 'lecture');
  _modSyncTools(on);
}
function modViewRename() {
  if (!_modViewCtx) return;
  const d = G.drills[_modViewCtx.i]; if (!d) return;
  renameChapter(d.id, _modViewCtx.k);
  _modViewTitle();   // le titre de l'écran suit le nouveau nom
}
...
// La ligne d'ouverture d'UN chapitre (la « partie » que le chapitre contient) :
// ses premiers demi-coups en figurines, la même signature que le répertoire —
// c'est elle qui distingue deux chapitres aux libellés proches. Numérotée depuis
// le vrai numéro de coup quand le chapitre part d'une position ([FEN]).
function _chapLine(d, k) {
  const pgn = chapterPgn(d, k);
  const sans = pgnMainlineSans(pgn, 6);
  if (!sans.length) return '';
  const sf = pgnStartFen(pgn).split(' ');
  const startBlack = sf[1] === 'b';
  let out = '', plain = '', n = parseInt(sf[5], 10) || 1;
  sans.forEach((san, j) => {
    const isWhite = startBlack ? j % 2 === 1 : j % 2 === 0;
    if (isWhite) { out += `<span class="mv-num">${n}.</span>`; plain += n + '.'; n++; }
    // Le coup NOIR de tête clôt son propre numéro : le coup blanc qui suit est
    // le suivant (« 4…a6 5.a4 »). Sans ce n++, le numéro se répétait et TOUTE
    // la fin de ligne était décalée d'un coup (« 4…a6 4.a4 e6 5.d4 »).
    else if (j === 0) { out += `<span class="mv-num">${n}…</span>`; plain += n + '…'; n++; }
    out += fig(escapeHtml(san)); plain += san;
    if (j < sans.length - 1) { out += ' '; plain += ' '; }
  });
  return `<span class="mdet-chap-line" title="${escapeHtml(plain)}">${out}</span>`;
}
...
function _mdetHTML({ d, i, due }) {
  ...
  const chaps = d.isExercise
    ...
    : sessions.map((s, k) => `
      <div class="mdet-chap-row">
        <button class="mdet-chap" onclick="openModuleView(${i},${k})"
                onmouseenter="_mdetShowChap('${d.id}',${k})" onfocus="_mdetShowChap('${d.id}',${k})"
                title="Ouvrir ce chapitre — lire la théorie, ✎ pour modifier">
          <span class="mdet-chap-txt">
            <span class="mdet-chap-label">${figurineTitle(escapeHtml(s.label || 'Chapitre ' + (k + 1)))}</span>
            ${_chapLine(d, k)}
          </span>
          <span class="mdet-chap-play"><i class="ti ti-book-2" aria-hidden="true"></i></span>
        </button>
        <button class="mdet-chap-edit" onclick="launchDrillChapter(${i},${k})"
                title="Tester ce chapitre comme un élève" aria-label="Tester le chapitre ${escapeHtml(s.label || 'Chapitre ' + (k + 1))}">
          <i class="ti ti-player-play" aria-hidden="true"></i>
        </button>
        <button class="mdet-chap-edit" onclick="openChapterTransfer('${d.id}',${k})"
                title="Copier ou déplacer ce chapitre vers un autre module"
                aria-label="Copier ou déplacer le chapitre ${escapeHtml(s.label || 'Chapitre ' + (k + 1))}">
          <i class="ti ti-transfer" aria-hidden="true"></i>
        </button>
        ${sessions.length > 1 ? `
        <button class="mdet-chap-edit" onclick="reorderChapter('${d.id}',${k},-1)" ${k === 0 ? 'disabled' : ''}
                title="Monter ce chapitre" aria-label="Monter le chapitre ${escapeHtml(s.label || 'Chapitre ' + (k + 1))}">
          <i class="ti ti-chevron-up" aria-hidden="true"></i>
        </button>
        <button class="mdet-chap-edit" onclick="reorderChapter('${d.id}',${k},1)" ${k === sessions.length - 1 ? 'disabled' : ''}
                title="Descendre ce chapitre" aria-label="Descendre le chapitre ${escapeHtml(s.label || 'Chapitre ' + (k + 1))}">
          <i class="ti ti-chevron-down" aria-hidden="true"></i>
        </button>` : ''}
      </div>`).join('')
      + `<button class="mdet-chap-add" onclick="openChapterPaste('${d.id}')">
           <i class="ti ti-clipboard-plus" aria-hidden="true"></i> Coller une partie
         </button>`;
  const facts = [
...
function renderDrillList() {
  ...
  if (!n) {
    ...
    grid.innerHTML = `<div class="mcard-empty">
      <div class="mcard-empty-ico">${mascotSvg(52, 'sleep')}</div>
  ...
  // Les DEUX cotes passent par _normTxt : le nom, et les mots du dossier-descripteur
  // (« Noirs exd4 ») qui sont ce qui distingue les 3 « Gambit Koltanowski ».
  if (_modQuery) list = list.filter(x => (_normTxt(x.d.name) + _normTxt(x.d.folder)).includes(_modQuery));
  ...
  const _sideGlyph = s => s === 'b' ? '♚' : s === 'both' ? '⇄' : '♔';
  const _sideName  = s => s === 'b' ? 'Noirs' : s === 'both' ? 'Les deux camps' : 'Blancs';
  const _rowHTML = ({ d, i, due, plies }, showSide = false) => {
    ...
    // Le compte de positions par rangee est retire (« pas important », 04/08) :
    // c'est un fait du module qu'aucune decision du coach n'utilise. Le nombre de
    // CHAPITRES reste — lui dit combien de lecons contient le module.
    const stat = (ns > 1 ? `<span class="mrow-chap"><b>${ns}</b> ch.</span>` : '')
               + (d.isExercise ? `<span><b>${nEx}</b> exos</span>` : '');
    // Les pastilles ne portent QUE des signaux d'action (à réviser, assigné,
    // échéance, type). Le dossier n'en est pas un : son numéro (« 760 ») ne
    // s'affiche plus nulle part sur la rangée (retiré le 22/07 à la demande de
    // la coach, comme les numéros de chapitres) — il reste dans le filtre dossier.
    const flags = [
      due > 0 ? `<button class="mrow-pill mrow-pill-due" onclick="event.stopPropagation();reviserDrill(${i})"><i class="ti ti-rotate" aria-hidden="true"></i> ${due} à réviser</button>` : '',
      ...
      d.plans?.length ? `<span class="badge" style="background:var(--cyan-dim);color:var(--cyan-ink)"><i class="ti ti-list-check" aria-hidden="true"></i> ${d.plans.length} plan${d.plans.length > 1 ? 's' : ''}</span>` : '',
    ...
    const editorItem = d.isExercise
      ...
      : `<button class="mcard-menu-item" onclick="openPgnEditor(${i})"><i class="ti ti-edit" aria-hidden="true"></i> Modifier</button>`;
    ...
    return `<div class="mrow${open ? ' open' : ''}" role="button" tabindex="0" aria-expanded="${open}"
      ...
      <div class="mrow-main">
        ...
        <div class="mrow-name" title="${escapeHtml(d.name)}">${showSide ? `<span class="mrow-side" title="${_sideName(d.side)}" aria-label="${_sideName(d.side)}">${_sideGlyph(d.side)}</span>` : ''}${figurineTitle(escapeHtml(d.name))}</div>
      ...
      <div class="mrow-acts">
        <button class="btn btn-primary btn-sm" onclick="launchDrill(${i})" title="Tester ce module comme un élève"><i class="ti ti-player-play" aria-hidden="true"></i> <span class="mrow-btn-lbl">Tester</span></button>
        ...
        <details class="mcard-menu" ontoggle="_mcardMenuAnchor(this)">
          ...
          <div class="mcard-menu-list">
            ${editorItem}
            ${d.isExercise ? '' : `<button class="mcard-menu-item" onclick="openPlanComposer('${d.id}')"><i class="ti ti-list-check" aria-hidden="true"></i> Plan de travail</button>`}
            <button class="mcard-menu-item" onclick="renameDrill('${d.id}')"><i class="ti ti-pencil" aria-hidden="true"></i> Renommer</button>
            <button class="mcard-menu-item" onclick="moveDrillToGroup('${d.id}')"><i class="ti ti-folders" aria-hidden="true"></i> Ranger dans un groupe</button>
  ...
  // Une section = ses rangées + un en-tête (glyphe, titre, comptes). `showSide`
  // ajoute le glyphe de camp à chaque rangée quand ce n'est PAS le camp qui
  // structure (sinon l'info du camp serait perdue).
  // Profondeur MAXIMALE de tout le répertoire affiché : une seule grille pour
  // toutes les sections (cf. la note dans _sectionHTML).
  const _lineW = (2 + 2.2 * list.reduce((a, x) => Math.max(a, x.plies || 6), 6)).toFixed(1);
  const _sectionHTML = (glyphHtml, title, rows, showSide) => {
    ...
    // ⚠ Largeur de colonne commune à TOUTE LA LISTE, pas à la section. Calculée
    // par section, « Répertoire Blancs » (6 demi-coups) et « Répertoire Noirs »
    // (8) recevaient deux grilles différentes : les coups ne démarraient pas au
    // même x d'une section à l'autre, et le balayage vertical — la raison d'être
    // de cette colonne — était cassé net entre les deux.
    return `<section class="mgroup" style="--mrow-linew:${_lineW}rem">
      <div class="mgroup-head">
        ${glyphHtml}
        <h2 class="mgroup-title">${title}</h2>
        <span class="mgroup-n">${rows.length} module${rows.length > 1 ? 's' : ''}</span>
      </div>
      ${rows.map(x => _rowHTML(x, showSide)).join('')}
    </section>`;
  };

  let sections;
  // La coach a rangé des modules dans des GROUPES nommés (« Répertoire Groupe 1 ») :
  // les groupes deviennent les sections (choix 29/07). Tant qu'aucun groupe n'existe,
  // on garde le tri par camp — la seule structure vraie d'un répertoire non organisé.
  const hasGroups = list.some(x => x.d.group);
  if (hasGroups) {
    const groupNames = [...new Set(list.map(x => x.d.group).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'fr'));
    const parts = groupNames.map(g =>
      _sectionHTML(`<span class="mgroup-side" aria-hidden="true"><i class="ti ti-folder"></i></span>`,
        escapeHtml(g), list.filter(x => x.d.group === g), true));
    const noGroup = list.filter(x => !x.d.group);
    if (noGroup.length) parts.push(_sectionHTML(
      `<span class="mgroup-side" aria-hidden="true"><i class="ti ti-folder-off"></i></span>`,
      'Sans groupe', noGroup, true));
    sections = parts.filter(Boolean).join('');
  } else {
    const SIDE_GROUPS = [
    ...
    sections = SIDE_GROUPS.map(g => _sectionHTML(
      `<span class="mgroup-side" aria-hidden="true">${g.glyph}</span>`, g.title,
      list.filter(x => (x.d.side || 'w') === g.key), false)).filter(Boolean).join('');
  }
  grid.className = '';
...
function renderClassList() {
  ...
  el.innerHTML = G.classes.map(cls => {
    ...
    const modNames = (cls.moduleIds || []).map(id => {
      ...
      // Une PASTILLE par module (03/08 : la phrase « Nom · date, Nom, Nom · date »
      // etait illisible) — l'echeance vit DANS la pastille de son module.
      if (!dl) return `<span class="cls-mod-chip">${nm}</span>`;
      let dlLabel = dl;
      try { dlLabel = new Date(dl + 'T00:00:00').toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }); } catch (e) {}
      const late = dl < new Date().toISOString().slice(0, 10);
      return `<span class="cls-mod-chip">${nm}<span class="cls-mod-date${late ? ' late' : ''}"><i class="ti ti-calendar" aria-hidden="true"></i> ${escapeHtml(dlLabel)}</span></span>`;
    });
    ...
    return `<div class="cls-card" role="button" tabindex="0" onclick="openClassDetail(${cls.id})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openClassDetail(${cls.id})}" aria-label="Ouvrir la classe ${escapeHtml(cls.individual ? cls.name.replace(/^👤\s*/,'') : cls.name)}">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="flex:1;min-width:0">
          ...
          ${modNames.length ? `<div class="cls-mod-chips">${modNames.slice(0,3).join('')}${modNames.length>3?`<span class="cls-mod-chip cls-mod-more">+${modNames.length-3}</span>`:''}</div>` : ''}
        </div>
        <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
          ...
          <i class="ti ti-chevron-right cls-card-go" aria-hidden="true"></i>
        </div>
      </div>
    </div>`;
  }).join('');
...
Object.assign(window, {
  ...
  modRowClick, modRowKey, launchDrillChapter, _mdetShowChap, renameChapter,
  openModuleView, closeModuleView, modToggleTools, modViewRename,
  _mcardMenuAnchor, _mcardMenuPrepare,
  modSearch, modFilterType, modSortBy, modSelectFolder, renameModFolder, moveDrillToFolder, moveDrillToGroup, openGroupModal, saveGroupModal, renameDrill,
