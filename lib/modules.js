...
const deleteModuleFromFirestore= (...a) => window.deleteModuleFromFirestore?.(...a);
...
// Stub temporaire — l'éditeur de position (FEN / échiquier vide) arrive juste après (tranche C).
function openPositionSetup() {
  window.toast?.('Éditeur de position — en cours d\'ajout', 'ok');
}
...
function renderCoachOnboarding() {
  ...
  const steps = [
    ...
    { done: nbClasses > 0, label: 'Créez une classe et ajoutez vos élèves',
      cta: `<button class="btn btn-gold btn-sm" onclick="switchCoachSection('eleves')">Créer une classe</button>` }
  ];
...
Object.assign(window, {
  openCreateDrillModal, openCreateChoice, openPositionSetup, openLibrary, renderLibrary, addFromLibrary,
