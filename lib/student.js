...
function _shModuleCard(m, idx, isNew, isPersonal) {
  ...
  if (m.deadline) {
    ...
    const st = diff < 0 ? ['var(--red-dim)','var(--red)','⚠ En retard']
             : diff <= 3 ? ['var(--gold-dim)','var(--gold)',`⏰ ${Math.round(diff)}j`]
             : ['var(--cyan-dim)','var(--cyan)',`📅 ${escapeHtml(m.deadline)}`];
  ...
  const icon = m.isExercise ? '🧩' : sideSym;
  const kindBadge = m.isExercise
    ? `<span class="sh-mod-side" style="background:var(--surf2);color:var(--violet)">🧩 ${nEx} exercice${nEx > 1 ? 's' : ''}</span>`
    : `<span class="sh-mod-side">${sideTxt}</span>`;
  const maiaBtn = m.isExercise ? '' : `<button class="sh-card-act" onclick="event.stopPropagation();playVsMaia(${idx})" title="Jouer contre Maia">🤖</button>`;
...
Object.assign(window, {
  ...
  startStudentDrill, openStudentImport, importStudentDrill, deleteStudentDrill,
});
