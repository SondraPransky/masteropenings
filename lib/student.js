...
function _shExerciseCard(m, idx, isNew) {
  ...
  if (m.deadline) {
    ...
    const d = diff < 0 ? ['var(--red-dim)','var(--red)','<i class="ti ti-alert-triangle" aria-hidden="true"></i> En retard']
    ...
    deadlineBadge = `<span class="sh-mod-side" style="background:${d[0]};color:${d[1]}">${d[2]}</span>`;
  }
  return `<div class="sh-mod${isNew ? ' sh-mod-new' : ''}" onclick="startStudentDrill(${idx})">
    ...
    <div class="sh-mod-actions">
      <button class="sh-card-act sh-card-play" title="S'entraîner"><i class="ti ti-player-play" aria-hidden="true"></i></button>
    </div>
  </div>`;
}
