...
function closeGameView() {
  ...
  const body = panel?.querySelector('.gv-body'); if (body) body.style.display = '';
...
// ── UN SEUL ÉCRAN lecture/annotation (03/08) ────────────────────────────────────
// « Voir la partie » ouvre l'ÉDITEUR ancré dans la section, en densité LECTURE
// (outils masqués, plateau inerte) : le calme de l'ancien lecteur, mais les
// variantes et commentaires du coach sont VISIBLES dès la lecture. ✎ révèle les
// outils dans le même écran — plus de 2e vue par-dessus.
function openGameDocked(pgn, meta = {}) {
  ...
  const panel = document.getElementById('pg-gameview'); if (panel) panel.style.display = '';
  const body = panel?.querySelector('.gv-body'); if (body) body.style.display = 'none';
