...
// En-tête PGN (`[White "…"]`) — fallback pour les parties enregistrées avant que
// white/black/event soient rangés dans extra (les noms restent dans le PGN).
function _pgnHeader(pgn, key) {
  if (!pgn) return null;
...
function _sbRowToGame(row) {
  ...
  return {
    ...
    white:        ex.white || _pgnHeader(row.pgn, 'White'),
    black:        ex.black || _pgnHeader(row.pgn, 'Black'),
    event:        ex.event || _pgnHeader(row.pgn, 'Event')
  };
}
