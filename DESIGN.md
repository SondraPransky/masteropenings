...
**La règle des chiffres — ⚠ `tabular-nums` est INERTE dans ce projet.** Mesuré le 23/07 : **Hanken Grotesk (la police du corps) a déjà des chiffres à chasse fixe par défaut.** `111111`, `888888` et `101010` rendent exactement la même largeur avec `font-variant-numeric: tabular-nums` et sans, à toutes les tailles. Les **17 déclarations `tabular-nums`** de `style.css` ne changent donc rien au rendu ; on les garde (assurance gratuite pour la fenêtre de FOUT, où le repli système afficherait des chiffres proportionnels) mais **ne pas en ajouter en croyant corriger un saut de layout** — il n'y en a pas. Le vrai levier d'alignement des chiffres reste le **choix de police** : JetBrains Mono pour les colonnes de données qu'on balaie verticalement.
...
- **Hover / Focus / Press:** hover = fond/bordure (120ms, propriétés explicites — jamais `transition: all`) ; focus = double anneau (`0 0 0 2px surf, 0 0 0 4px indigo-glow`) ; press = `scale(.96)` (.98 sur grandes cartes).
  **Le press-scale vaut pour TOUT contrôle interactif**, pas seulement les `.btn` : chips (`.oaa-chip`), pastilles (`.exp-mvpill`, `.exp-levelchip`), crans de rail (`.oaa-rail-stop`), actions de carte (`.sh-card-act`). Deux contrôles côte à côte qui ne répondent pas pareil sous le doigt, ça se sent. ⚠ `transform` doit être **dans la liste de `transition`** du composant, sinon le scale claque. Le press-scale n'est **pas** désactivé sous `prefers-reduced-motion` (c'est un retour tactile instantané, pas une animation décorative) — convention du projet, la suivre plutôt que d'inventer une exception locale.
  ⚠ **Exception : `.mrow`.** Une rangée de répertoire n'a NI hover-transform NI press-scale. Un `transform` y crée un contexte d'empilement, qui piège le menu ⋯ en `position:fixed` — le bug des 4 passes de juillet. Le problème est supprimé par construction, pas contourné.
- **Icône seule:** toujours `aria-label` + classe `.btn-ico` (pseudo-élément → cible 40px). Sur tactile (`@media (hover:none)`), `.btn` **et** `.btn-sm` remontent à `min-height: 44px`.
- **Bascule d'icône** (soleil ⇄ lune, œil ⇄ œil barré) : le glyphe entrant s'anime — `scale .25→1`, `opacity 0→1`, `blur 4px→0`, `.22s cubic-bezier(.2,0,0,1)` via `.ico-swap`. ⚠ La classe n'est posée que sur le **chemin du clic**, jamais à l'init : sinon l'animation rejoue à chaque chargement de page et devient du bruit.
...
- **Do** écrire les noms d'ouverture dérivés d'une **nationalité ou d'un pays avec une majuscule** — « la Scandinave », « l'Italienne », « la Sicilienne », « la Française », « l'Espagnole ». C'est le nom propre d'une ouverture, pas un adjectif : la majuscule fait partie du nom (convention échiquéenne française, respectée partout dans le fonds de la coach). Vaut pour les noms de modules, les libellés, les toasts et tout texte généré.
