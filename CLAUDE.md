...
| Fichier | Rôle | Taille |
|---------|------|--------|
| `app.js` | Cœur applicatif : login/auth Supabase, accès Supabase (`_sb*`), init, helpers (`escapeHtml`/`fig`/`toast`/`setFeedback`/`updateScores`…), `currentGame`/`isLineMode`, `save`/`saveClasses`/`goPage`, pont `window` | ~56 Ko · 1139 lignes |
...
>   - ⚠️ **Bug pré-existant repéré (non corrigé, hors périmètre)** : annuler une promotion sur le **board principal** via `#promo-backdrop` → `cancelPromo` appelle `window.renderEditorBoard?.()` (editor.js), qui fait `new Chess(_E.node.fenAfter)` **sans garder `_E.node`** → `TypeError` si l'éditeur PGN n'a jamais été ouvert (`_E.node` null). La partie visible (masquer le sélecteur) a déjà eu lieu avant le throw. Antérieur au refactor (`cancelPromo` appelait déjà `renderEditorBoard`). Fix trivial : garde `if (!grid || !_E?.node) return;` dans `renderEditorBoard` (lib/editor.js).
> - **§5.3 TERMINÉ** : `app.js` ne contient plus que login/auth Supabase, la couche `_sb*`, l'init, les helpers UI (`escapeHtml`/`fig`/`toast`/`setFeedback`/`updateScores`/`addLog`…), `currentGame`/`isLineMode`, `save`/`saveClasses`/`goPage` et le pont `window`. Depuis le début du refactor : `app.js` 4412 → 1139 lignes (−74 %).
