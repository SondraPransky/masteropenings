...
- **D — Action « Voir la position »** : modale `modal-weakspot` (grand échiquier + coup + commentaire + élèves qui échouent), `openWeakspotPosition` + stash `_wsCards`. **« Assigner une révision ciblée » NON faite** : bloquée par l’architecture multi-compte (le coach ne peut pas écrire la SR d’un élève ; nécessite un vrai modèle d’**assignation** de position, surface côté élève → post-scope).
