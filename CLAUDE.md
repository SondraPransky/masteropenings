...
**État au 17/07 (mesuré après `fetch`, Action Pages verte) : `main` == `origin/main`, tout est poussé** — les 3 commits du 17/07 (`46aaeab` · `02e465e` · `456836d`) sont sur `origin`. Reste vrai : **la couche d'édition élève n'a jamais tourné en connecté** — c'est le seul vrai risque ouvert.

> ⚠️ **Cette ligne s'est périmée TROIS fois** (elle a annoncé « non poussé » des commits déjà sur `origin`, puis « non committé » un travail committé). **Ne jamais s'y fier — la mesurer**, après un `git fetch` (sans quoi `origin/main` est une référence locale périmée) :
