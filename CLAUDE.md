...
| Fichier | Rôle | Taille |
|---------|------|--------|
| `app.js` | Cœur applicatif : login, échiquier (canvas, drag, dispatch), accès Supabase, `save`/`saveClasses`/`goPage`, pont `window` | ~68 Ko · 1448 lignes |
...
> - **Prochaine étape** : l'échiquier (canvas, drag, `tryMove`/`canInteract`, `showPromoPicker`, `_lastMoveXY`) → `lib/board.js`. Désormais quasi-contigu (601-687 + 689-… après retrait de Maia). ⚠️ Zone sensible : listeners `#board` attachés au **chargement du module** (ordre d'import) ; `tryMove` est le hub de dispatch (→ `drill.js`/`maia.js` via `window`).
